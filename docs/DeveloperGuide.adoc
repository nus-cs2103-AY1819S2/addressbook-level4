...
.. It first tests if the `ModuleInfoCode` (call it `codeToTest`) in a `RecModule` is eligible to be read,
using an `EligibleModulePredicate` which takes in `GradTrak`. Those `RecModule` corresponding to `ModuleTaken` in `GradTrak`
which are already passed or to be read in a future semester (collectively known as non-failed modules) are filtered out at this stage.
...
considering that lower level modules are usually read first. In the case of equal priority and module level, lexicographical sorting is used.
