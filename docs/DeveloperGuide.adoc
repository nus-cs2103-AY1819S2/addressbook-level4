...
Step 2. `ActivityAddCommand` would be executed and it would attempt to add the new `Activity` to `Model`. +
...
The `activityFilter` command enables users to filter activities based on the time
