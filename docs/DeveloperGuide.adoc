...
Alternative 1.1: OpenCommand -> Json Util -> FileUtil
Alternative 1.2: OpenCommand -> FileUtil
...
* **Alternative 1a:** Save .json file by calling JsonUtil or FileUtil.
Current implementation: SaveCommand -> InOutAddressBookStorage -> JsonUtil -> FileUtil
** Pros: Less overhead and faster runtime as there are less classes to go through.
Alternative 1a.1: SaveCommand -> Json Util -> FileUtil
Alternative 1a.2: SaveCommand -> FileUtil
** Cons: InOutAddressBookStorage does some error handling. Bypassing InOutAddressBookStorage would require the same error handling in SaveCommand. Since SaveCommand is not called when the program starts, we cannot move the error handling from InOutAddressBookStorage to SaveCommand. Then we would have to copy the error handling instead, which means that we now have a duplicate logic.

* **Alternative 1b:** Save .pdf file by calling PdfUtil or FileUtil.
Current implementation: SaveCommand -> InOutAddressBookStorage -> PdfUtil
** Pros: Less overhead and faster runtime as there are less classes to go through.
Alternative 1b.1: SaveCommand -> Pdf Util -> FileUtil
Alternative 1b.2: SaveCommand -> FileUtil
Alternative 1b.3: SaveCommand -> PdfUtil
