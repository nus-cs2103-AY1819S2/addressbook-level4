...
==== *Adding a request*: `add request/r/2 n/<patient_name> c/<condition> dt/<date> i/<NRIC> p/<phone> a/<address>`
...
After completing all existing requests, you also have the option of clearing all
 requests from the request list, upon
  entering the `clr` command. Note that the clear command can only be issued if every request in the request list is completed.
