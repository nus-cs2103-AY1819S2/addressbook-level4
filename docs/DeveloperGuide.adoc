...
Step 5. The user then decides to execute the command `list r`. Commands that do not modify the address book, such as `list r`, will not call `Model#commit()`, `Model#undo()` or `Model#redo()`. Thus, the `VersionedBook` s do not change.
...
|Cons| Unable to handle commands that change multiple books at once. We assume that only one `currentStatePointer` can change per undo/redo.| Keeping a pair of integers takes up more memory as compared to keeping a single enum.
|=======================================================================
