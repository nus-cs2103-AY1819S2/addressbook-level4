...
** Cons: InOutAddressBookStorage does some error handling. Bypassing InOutAddressBookStorage would require the same error handling in SaveCommand. Since SaveCommand is not called when the program starts, we cannot move the error handling from InOutAddressBookStorage to SaveCommand. In that case, we would have to copy the error handling instead, which means that we now have a duplicate logic, which is also not ideal.
