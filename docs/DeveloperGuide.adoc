...
The File Management features are: `Open`, `Save`, `Import`, `Export`
...
4. ExportCommand checks for if the requested file is a ".json" file, if the file exists, if it is a file, or if it can be read. +
5a. ExportCommand add contents from the current storage to the temporary storage based on the input INDEX_RANGE. +
    5b. ExportCommand calls SaveCommand if INDEX_RANGE is `all`. Refer to Save feature. +
