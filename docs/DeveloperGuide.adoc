...
[NOTE]
This action can only be done in Menu Mode. If not in Menu Mode currently , users can first change mode by typing `MenuMode` or `MM` in the command box. The change in mode will be apparent from the change in User Interface and the success message in the result display box. Such constraints are implemented in order to change the User Interface for clarity for the users.
