...
absolute file path of the image as an `add` and `edit` command parameter, specified by the prefix `p/`.

<<Figure 4.4.1>> below is a sequence of steps, illustrating the interaction between various classes when the add command is
used to include a photo with a newly created place.

[[AddPhotoActivityDiagram]]
[reftext="Figure 4.2.1.1"]
[caption="Figure 4.4.1: Add Photo Activity Diagram"]
...
Step 2 - 5: Identical to `add` command sequence, please refer to <<<Add Feature>>> for details

Step 6: The `AddCommandParser` interprets the arguments, and checks the file path input using `ParserUtil.parsePhoto()`.

Step 7: `parsePhoto()` then trims any leading whitespace. It also trims a single leading and trailing double quotation mark `"`.
...
Step 8.2.1: If photo prefix 'p/' is used, he validity of the filepath is then checked with the `Photo.isValidPhotoFilepath()` method.
...
Step 8.2.2: Else, control returns to `parseUtil.parsePhoto()`. It uses the all-parameter `Place()` constructor to create
