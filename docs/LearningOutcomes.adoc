...
Note how components of CardFolder have well-defined APIs. For example, the API of the `Logic` component is given in the link:{repoURL}/src/main/java/seedu/address/logic/Logic.java[`Logic.java`]
...
Note how the CardFolder app uses Java ``assert``s to verify assumptions.
...
* Add more assertions to CardFolder as you see fit.
...
Note <<DeveloperGuide#logging, how the CardFolder app uses Java's `java.util.log` package to do logging>>.
...
Add more logging to CardFolder as you see fit.
...
Note how CardFolder uses the `ReadOnly*` interfaces to prevent objects being modified by clients who are not supposed to modify them.
...
Analyze the CardFolder code/design to identify,
...
Note <<UsingGradle#, how the CardFolder app uses Gradle to automate build tasks>>.
...
Note <<UsingTravis#, how the CardFolder app uses Travis to perform Continuous Integration>>. (https://travis-ci.org/se-edu/addressbook-level4[image:https://travis-ci.org/se-edu/addressbook-level4.svg?branch=master[Build Status]])
...
* Test methods `prefsReadSave()` and `cardFolderReadSave()` are integration tests. Note how they simply test if The `StorageManager` class is correctly wired to its dependencies.
* Test method `handleCardFolderChangedEvent_exceptionThrown_eventRaised()` is a unit test because it uses _dependency injection_ to isolate the SUT `StorageManager#handleCardFolderChangedEvent(...)` from its dependencies.
...
** Sub classes of link:{repoURL}/src/main/java/seedu/address/ui/UiPart.java[`UiPart`] (e.g. `CardListPanel` ) act as 'Controllers', each controlling some part of the UI and communicating with the 'Model' (via the `Logic` component which sits between the 'Controller' and the 'Model').
