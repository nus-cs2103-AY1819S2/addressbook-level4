...
* Words in `UPPER_CASE` are the parameters to be supplied by you e.g. in `addfolder FOLDER_NAME`, `FOLDER_NAME` is a parameter which can be used as `add Human Anatomy`.
...
Creates a folder with the name "Nervous System". You can then enter the folder with the `cd` command and begin adding cards.
...
You will enter a test session, where the display area enters a fullscreen and you will be presented flashcard questions
 one by one, as depicted in the figure below.
...
* `ans 2` +
in response to the card displayed below, where choosing option 2 will give the right answer as the correct answer is
'Pigs'.
...
The report comprises a graph showing the last 10 <<testscore, test scores>>,
the latest score change, and the top 3 lowest individual scoring cards and their individual <<cardscore, card scores>>.
