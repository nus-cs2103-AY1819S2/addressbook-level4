...
`edit <keyword> <parameter> [<parameter>]...` +
...
valid first keyword and argument prefixes and values.
...
composing the `Predicate` that were formed from the user inputs into a single
 ...
 `Predicate` library. +
This is because maintain a `List` of `Predicate` in HealthHub increases the
abstraction use in implementing any functionality that involves a `Predicate`
 of a field, and accessing the inner `Predicate` in the list would mean
 passing through an additional layer of abstraction and violating the Law of
 Demeter.
...
In addition, by returning a single `Predicate` object, ease of implementation
 ...
 needs only to depend on `Predicate` and not `List`.
