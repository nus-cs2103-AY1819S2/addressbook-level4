...
[NOTE]
====
After the `start` command starts successfully, BrainTrain will switched to <<quizmode, `Quiz
Mode`>> and you can only enter quiz mode commands.
====
// tag::evan[]
[[listLessons]]
...
When you <<start, `start`>> a quiz, you will enter quiz mode in which only the following set of quiz commands are allowed. Management commands are disabled in this mode.
