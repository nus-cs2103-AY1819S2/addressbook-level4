...
** Pros: Users can export directly through a single command (e.g. for `find` command when the export option is enabled, export the filtered medicine inventory data immediately without having to retrieve from `Model#getFilteredMedicineList()` operation.
