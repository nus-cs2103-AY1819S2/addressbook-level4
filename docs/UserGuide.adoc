...
Step 1: List all the restaurants by typing `list`, then pressing enter, as shown below.

image::setcategories-stepone.png[width="800"]
...
is identified by *INDEX 1*. Start typing `setCategories 1 c/` as follows:

image::setcategories-steptwo.png[width="800"]
...
Follow Figure 3 shown below.

image::setcategories-stepthree.png[width="800"]
_Figure 3: Once again, you can pick the occasion you see in the suggestions or continue typing._

Step 4: Key in the price range next, as demonstrated by Figure 4 below.

image::setcategories-stepfour.png[width="800"]
...
following status message shown in the Figure below.

image::setcategories-stepfive.png[width="800]
_Figure 5: Success!_
...
Step 1: Suppose you want to filter out all the `Western` restaurants. Type `filter western` as shown below.

image::filter-stepone.png[width="800"]
_Figure 1: Type `filter western` into the command box_

Step 2: Press enter and you should see all the western restaurants. Refer to Figure 2 for a visual representation.

image::filter-steptwo.png[width="800"]
_Figure 2: Western restaurants are listed_

Step 3: Suppose now you want to filter out all `Western`, `Japanese`, as well as `Chinese` restaurants. Type `filter
western japanese chinese` as shown below.

image::filter-stepthree.png[width="800"]
_Figure 3: Type `filter western japanese chinese` into the command box_

Step 4: Press enter again and now all `Western`, `Japanese` and `Chinese` restaurants will be shown. See Figure 4
below for example.

image::filter-stepfour.png[width="800"]
_Figure 4: Western, Japanese and Chinese restaurants are now listed_
...
* `filter fine dining` +
Returns restaurants with any category matching `fine` or `dining`.
* `filter fine dining casual`
Returns restaurants with any category matching `fine`, `dining` or `casual`.
* `filter $`
Returns restaurants with any category matching `$`. In this case will most likely match `Price Range`.
