...
=== Adding a ship: `add ship`

Adds a battleship to the player's inventory. There is a maximum number of battleships that a player may have. +
Format: `add ship n/NAME s/SIZE c/COMMAND_FLAG t/TAGS`

[NOTE]
There is a maximum size that all of the player's battleships may add to.

[NOTE]
The command battleship must be only of size 3. If the command battleship has been hit the player will lose more points. Each player must have only 1 command battleship.

Examples:

* `add ship n/RSS Vengence s/4 c/true t/bluewater_fleet`
* `add ship n/RSS Putra s/1 c/false`
...
=== Placing a ship: `put ship`

Adds a ship onto the player's map. +
Format: `put ship x/ROW_COORDINATE y/COL_COORDINATE r/ORIENTATION`

Examples:

* `put ship x/5 y/2 r/horizontal`
* `put ship x/1 y/4 r/vertical`
...
Format: `shoot x/X_COORD y/Y_COORD`
...
* **History**: `history`
* **Stats**: `stats`
* **Shoot**: `shoot x/X_COORD y/Y_COORD`
* **Play**: `play`
* **Select**: `select INDEX`
* **Delete**: `delete INDEX`
* **Find Tag**: `findTag TAG [MORE_TAGS]`
* **Find**: `find NAME [MORE_NAMES]`
* **Put**: `put ship x/ROW_COORDINATE y/COL_COORDINATE r/ORIENTATION`
* **Edit**: `edit INDEX [n/NAME] [s/SIZE] [c/COMMAND_FLAG] [t/TAGS]`
* **List**: `list`
* **Help**: `add ship n/NAME s/SIZE c/COMMAND_FLAG t/TAGS`
