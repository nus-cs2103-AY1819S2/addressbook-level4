...
*** `Model#setCardAsNotAnswered()`
*** `Model#checkIfCardAlreadyAnswered()`

==== Test / End Command

[discrete]
===== Model
The main logic for  `test` and `end` command is carried out inside `ModelManager`,
updating the 4 variables introduced earlier that are used to track the state of a test session accordingly,
with the following methods:

* `#testCardFolder()` - begins a test session on the current card folder user is in.
** `currentTestedCardFolder` is set to the current folder the user is in.
** `Model#setCurrentTestedCard()` is invoked to set `currentTestedCard` to the first card in the folder as `currentTestedCardIndex` is set to 0.
** `isInsideTestSession` is set to true.
** `isCardAlreadyAnswered`` is by default false.
...
** `isInsideTestSession` is set to false.
** `isCardAlreadyAnswered` is reset to false.
