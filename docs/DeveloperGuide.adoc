...
. Repeat this for the test folder as well (e.g. check link:{repoURL}/src/test/java/seedu/knowitall/ui/HelpWindowTest.java[`HelpWindowTest.java`] for code errors, and if so, resolve it the same way)
...
image::ModelManagerUpdateDiagram.png[width="800"]
...
* `ModelManager#setActiveCardFolderIndex()` - Specifies the active `CardFolder` for operations to be performed on via the index in `ModelManager`’s list
...
image::StorageManagerUpdateDiagram.png[width="800"]

Notable new methods:

* `StorageManager#readCardFolders()` - Reads in all `CardFolders` from all `CardFolderStorage` objects in the list.
* `StorageManager#saveCardFolders(List<ReadOnlyCardFolder> cardFolders)` - Saves all `CardFolder`s provided in the argument.

===== Logic
The logic component propagates changes in a `CardFolder` to the Storage component. With listeners, it is informed when a `CardFolder` is modified (e.g. a new card is added) so that it can invoke the appropriate Storage methods.

The same principle was applied to propagate changes in `CardFolders` (e.g. adding a new folder) to Storage. Model is now an `Observable`, and changes to a Model’s CardFolders will inform the LogicManager, which in turn invokes the `StorageManager#saveCardFolders(List<ReadOnlyCardFolder> cardFolders)` method.

==== Design Considerations
===== Aspect: How multiple folders are represented
...
* **Alternative 1 (current choice)**: Use a unique folder name
** Pros: Easier to implement.
** Cons: Have to use a separate test for comparing folders across time (which necessarily have to compare cards).
...
****
* Hints
** Just like we store each individual command word constant `COMMAND_WORD` inside `*Command.java` (e.g.  link:{repoURL}/src/main/java/seedu/knowitall/logic/commands/FindCommand.java[`FindCommand#COMMAND_WORD`], link:{repoURL}/src/main/java/seedu/knowitall/logic/commands/DeleteCommand.java[`DeleteCommand#COMMAND_WORD`]), you need a new constant for aliases as well (e.g. `FindCommand#COMMAND_ALIAS`).
** link:{repoURL}/src/main/java/seedu/knowitall/logic/parser/CardFolderParser.java[`CardFolderParser`] is responsible for analyzing command words.
* Solution
** Modify the switch statement in link:{repoURL}/src/main/java/seedu/knowitall/logic/parser/CardFolderParser.java[`CardFolderParser#parseCommand(String)`] such that both the proper command word and alias can be used to execute the same intended command.
...
****
* Hints
** The link:{repoURL}/src/main/java/seedu/knowitall/model/Model.java[`Model`] and the link:{repoURL}/src/main/java/seedu/knowitall/model/CardFolder.java[`CardFolder`] API need to be updated.
** Think about how you can use SLAP to design the method. Where should we place the main logic of deleting hints?
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/knowitall/model/CardFolder.java[`CardFolder`] and link:{repoURL}/src/main/java/seedu/knowitall/model/card/Card.java[`Card`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/knowitall/model/CardFolder.java[`CardFolder`] allows you to update a card, and link:{repoURL}/src/main/java/seedu/knowitall/model/card/Card.java[`Card`] allows you to update the hints.
* Solution
** Implement a `removeTag(Tag)` method in link:{repoURL}/src/main/java/seedu/knowitall/model/CardFolder.java[`CardFolder`]. Loop through each card, and remove the `tag` from each card.
** Add a new API method `deleteTag(Tag)` in link:{repoURL}/src/main/java/seedu/knowitall/model/ModelManager.java[`ModelManager`]. Your link:{repoURL}/src/main/java/seedu/knowitall/model/ModelManager.java[`ModelManager`] should call `CardFolder#removeTag(Tag)`.
...
****
* Hints
** The tag labels are created inside link:{repoURL}/src/main/java/seedu/knowitall/ui/CardCard.java[the `CardCard` constructor] (`new Label(tag.hintName)`). https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/Label.html[JavaFX's `Label` class] allows you to modify the style of each Label, such as changing its color.
...
. Modify link:{repoURL}/src/main/java/seedu/knowitall/commons/events/ui/NewResultAvailableEvent.java[`NewResultAvailableEvent`] such that link:{repoURL}/src/main/java/seedu/knowitall/ui/ResultDisplay.java[`ResultDisplay`] can show a different style on error (currently it shows the same regardless of errors).
+
**Before**
+
...
****
* Hints
** link:{repoURL}/src/main/java/seedu/knowitall/commons/events/ui/NewResultAvailableEvent.java[`NewResultAvailableEvent`] is raised by link:{repoURL}/src/main/java/seedu/knowitall/ui/CommandBox.java[`CommandBox`] which also knows whether the result is a success or failure, and is caught by link:{repoURL}/src/main/java/seedu/knowitall/ui/ResultDisplay.java[`ResultDisplay`] which is where we want to change the style to.
** Refer to link:{repoURL}/src/main/java/seedu/knowitall/ui/CommandBox.java[`CommandBox`] for an example on how to display an error.
* Solution
** Modify link:{repoURL}/src/main/java/seedu/knowitall/commons/events/ui/NewResultAvailableEvent.java[`NewResultAvailableEvent`] 's constructor so that users of the event can indicate whether an error has occurred.
** Modify link:{repoURL}/src/main/java/seedu/knowitall/ui/ResultDisplay.java[`ResultDisplay#handleNewResultAvailableEvent(NewResultAvailableEvent)`] to react to this event appropriately.
...
. Modify the link:{repoURL}/src/main/java/seedu/knowitall/ui/StatusBarFooter.java[`StatusBarFooter`] to show the total number of people in the card folder.
+
**Before**
+
...
** link:{repoURL}/src/main/java/seedu/knowitall/ui/StatusBarFooter.java[`StatusBarFooter`] needs to initialize the status bar on application start, and to update it accordingly whenever the card folder is updated.
* Solution
** Modify the constructor of link:{repoURL}/src/main/java/seedu/knowitall/ui/StatusBarFooter.java[`StatusBarFooter`] to take in the number of cards when the application just started.
** Use link:{repoURL}/src/main/java/seedu/knowitall/ui/StatusBarFooter.java[`StatusBarFooter#handleCardFolderChangedEvent(CardFolderChangedEvent)`] to update the number of cards whenever there are new changes to the knowitall.
...
****
* Hint
** Add the API method in link:{repoURL}/src/main/java/seedu/knowitall/storage/CardFolderStorage.java[`CardFolderStorage`] interface.
** Implement the logic in link:{repoURL}/src/main/java/seedu/knowitall/storage/StorageManager.java[`StorageManager`] and link:{repoURL}/src/main/java/seedu/knowitall/storage/JsonCardFolderStorage.java[`JsonCardFolderStorage`] class.
* Solution
...
. Add a `RemarkCommand` that extends link:{repoURL}/src/main/java/seedu/knowitall/logic/commands/Command.java[`Command`]. Upon execution, it should just throw an `Exception`.
. Modify link:{repoURL}/src/main/java/seedu/knowitall/logic/parser/CardFolderParser.java[`CardFolderParser`] to accept a `RemarkCommand`.

**Tests:**

. Add `RemarkCommandTest` that tests that `execute()` throws an Exception.
. Add new test method to link:{repoURL}/src/test/java/seedu/knowitall/logic/parser/CardFolderParserTest.java[`CardFolderParserTest`], which tests that typing "remark" returns an instance of `RemarkCommand`.
...
. Modify link:{repoURL}/src/main/java/seedu/knowitall/logic/parser/CardFolderParser.java[`CardFolderParser`] to use the newly implemented `RemarkCommandParser`.
...
. Modify link:{repoURL}/src/test/java/seedu/knowitall/logic/parser/CardFolderParserTest.java[`CardFolderParserTest`] to test that the correct command is generated according to the user input.

===== [Step 3] Ui: Add a placeholder for remark in `CardCard`
Let's add a placeholder on all our link:{repoURL}/src/main/java/seedu/knowitall/ui/CardCard.java[`CardCard`] s to display a remark for each card later.
...
. Add FXML annotation in link:{repoURL}/src/main/java/seedu/knowitall/ui/CardCard.java[`CardCard`] to tie the variable to the actual label.
...
We have to properly encapsulate the remark in our link:{repoURL}/src/main/java/seedu/knowitall/model/card/Card.java[`Card`] class. Instead of just using a `String`, let's follow the conventional class structure that the codebase already uses by adding a `Remark` class.

**Main:**

. Add `Remark` to model component (you can copy from link:{repoURL}/src/main/java/seedu/knowitall/model/card/Address.java[`Address`], remove the regex and change the questions accordingly).
...
Now we have the `Remark` class, we need to actually use it inside link:{repoURL}/src/main/java/seedu/knowitall/model/card/Card.java[`Card`].

**Main:**

. Add `getRemark()` in link:{repoURL}/src/main/java/seedu/knowitall/model/card/Card.java[`Card`].
...
. Modify link:{repoURL}/src/main/java/seedu/knowitall/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `data/cardfolder.json` so that the application will load the sample data when you launch it.)

===== [Step 6] Storage: Add `Remark` field to `JsonAdaptedCard` class
We now have `Remark` s for `Card` s, but they will be gone when we exit the application. Let's modify link:{repoURL}/src/main/java/seedu/knowitall/storage/JsonAdaptedCard.java[`JsonAdaptedCard`] to include a `Remark` field so that it will be saved.
...
Since `Card` can now have a `Remark`, we should add a helper method to link:{repoURL}/src/test/java/seedu/knowitall/testutil/CardBuilder.java[`CardBuilder`], so that users are able to create remarks when building a link:{repoURL}/src/main/java/seedu/knowitall/model/card/Card.java[`Card`].

**Tests:**

. Add a new method `withRemark()` for link:{repoURL}/src/test/java/seedu/knowitall/testutil/CardBuilder.java[`CardBuilder`]. This method will create a new `Remark` for the card that it is currently building.
. Try and use the method on any sample `Card` in link:{repoURL}/src/test/java/seedu/knowitall/testutil/TypicalCards.java[`TypicalCards`].

===== [Step 7] Ui: Connect `Remark` field to `CardCard`
Our remark label in link:{repoURL}/src/main/java/seedu/knowitall/ui/CardCard.java[`CardCard`] is still a placeholder. Let's bring it to life by binding it with the actual `remark` field.

**Main:**

. Modify link:{repoURL}/src/main/java/seedu/knowitall/ui/CardCard.java[`CardCard`]'s constructor to bind the `Remark` field to the `Card` 's remark.

**Tests:**

. Modify link:{repoURL}/src/test/java/seedu/knowitall/ui/testutil/GuiTestAssert.java[`GuiTestAssert#assertCardDisplaysCard(...)`] so that it will compare the now-functioning remark label.
