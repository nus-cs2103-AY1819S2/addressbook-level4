...
[WARNING]
The *WARNING* symbol is used to warn you of something.
// end::additionalInfo[]
...
*Description:* :: 
The `add` command adds a place to TravelBuddy, as seen in <<addFigure>>.
...
*Description:* ::
The `list` command displays a list of all the places in TravelBuddy, as seen in <<listFigure>> below.
...
[NOTE]
Calling the `list` command returns a list of all the places in TravelBuddy as shown in the image below.
...
*Description:* :: The `edit` command edits an existing place in TravelBuddy. A comparision can be made between <<editFigureBefore>> and <<editFigureAfter>>
...
From Figure 3.6.1, using `search Singapore` will return all places in your TravelBuddy that contain `Singapore` in its name. + 
...
From Figure 3.7.1, using `searchr 4` will return all places in your TravelBuddy that have a rating of `4`.
****
...
From Figure 3.8.1, using `searcht distillery` will return all places in your TravelBuddy that are tagged with `distillery`.
****
...
From Figure 3.9.1, using `searchc JPN` will return all places in your TravelBuddy that are located in Japan.
****
...
From Figure 3.10.1, using `searchyear 2016` will return all places in your TravelBuddy you visited in the year `2016`.
****
...
The image below shows TravelBuddy before `delete` command was used.
...
[[chart1Figure]]
.Number of Places Visited for Each Country
image::chart1.png[width="600"]

[[chart2Figure]]
.Number of Places Visited for Each Rating Category
image::chart2.png[width="600"]

[[chart3Figure]]
.Number of Places Visited for Each Year
...
* The chart will not display anything when the list is empty. See <<generateCommand6Figure>>
****
...
_Outcome:_ <<generateCommand3Figure>> +
* `clear` + 
<<generateCommand4Figure>> + 
`generate` + 
<<generateCommand5Figure>>  + 
_Outcome:_ <<generateCommand6Figure>>
****
...
[[exitFigure]]
.An alternative way to exit TravelBuddy is to click kbd:[FILE] > kbd:[EXIT]
