...
Although the selected option require more user input and involve lengthier commands, doctors can circumvent
these pitfalls very easily.

Using the current implementation, doctors can recover from erroneous commands very quickly by making use of the command history.
They can also perform cross module operations during a consultation. As such, the selected implementation guarantees the flexibility
and efficiency QuickDocs aim to deliver for doctors in neighbourhood clinics.
