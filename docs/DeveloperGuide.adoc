...
The _Sequence Diagram_ below shows how the components interact with each other for the scenario where the user issues the command `attack a1`.

.Component interactions for `delete 1` command
image::HighLevelAttackDiagram.png[width="800"]
...
image::ShipManagementUseCaseDiagram.png[]
...
`populateMapGrid()` will generate randomised ships based on the number of ships
...
image::inputStatsCommandSS.png[width=200]

Pop-up window for `stats` :

.Screenshot of empty statistics pop-up window.
image::StatsWindowSS.png[width=300]
...
image::StatsSequenceDiagram.PNG[width=500 ]
...
image::StatsWinScreen.PNG[width=500]
...
image::SaveSequenceDiag.PNG[width=350]
...
image::StatsUseCaseDiag.PNG[width=150]
...
|`* * *` | player | put a ships on the map | play the game

|`* * *` | player | see where my ships are on the map |have a visual representation of the game

|`* * *` | player | tag my ships | manage my ships during the game

|`* * *` | player | list my ships | check the status of my ships during the game

|`* * *` | player | list my ships by tag | check the status of specific ships during the game
...
2. Shows all of the user's ships deployed on map.
+
Use case ends.
...
=== List all tags that ships have

*MSS*

1. User requests to list tags of ships that have already been deployed on map.
2. Shows all of the tags of ships that have already been deployed on map.
+
Use case ends.
...
2. Shows all of the user's ships deployed on map that have certain tags.
+
Use case ends.
