...
The client card will shows its client name and the number of equipment this client owns.
...
* `sort date`
Returns the list sorted according to date.
...
[TIP]
* The user can filter the current shown list with any specified fields, and can filter by multiple fields.
