...
Windows developers are highly recommended to use JDK `9` to retain consistency.
...
. Open a console and run the command `gradlew processResources`. For Mac/Linux environments, run `./gradlew processResources`). It should finish with the [green]`BUILD SUCCESSFUL` message. +
...
. Run the `seedu.address.MainApp` and try a few basic commands.
...
The UI consists of a `MainWindow` that is made up of different UI components e.g.`CommandBox`, `ResultDisplay`, `RequestListPanel`, `InfoPanel`, `HealthWorkerListPanel`, `StatusBarFooter`, etc.

All of these, including the `MainWindow`, inherit from the `UiPart` class, which is an abstract class containing methods to load FXML files.
...
The `add`, `edit` and `delete` commands have been tweaked from the original AB4
 to include addition of `Request` and `HealthWorker`.

==== Additional Implementation from v1.1
...
* Each of the `add`, `edit` and `delete` command is parsed through the
`EditCommandParser`, `AddCommandParser` and `DeleteCommandParser` classes,
where the `CommandMode` is parsed to return the appropriate type of `Command`
 class:
** `AddCommand`
** `EditCommand`
** `DeleteCommand`

* Each of the `AddCommand` and `EditCommand` and `DeleteCommand` provides
subclasses for each of the `Command` classes respective to `Patient`,
`HealthWorker` and `Request` class which adds respective objects to
`PatientBook`, `RequestBook` and `HealthWorkerBook` for storage, for example
the subclasses for `AddCommand` for `Request` are as follows:
** `AddRequestCommand`
** `EditRequestCommand`
...
The select request feature allows the administrator to view a specific request by keying in the index of the request.
The select request mechanism is facilitated by the `filteredRequestList` in the `ModelManager`. The `filteredRequestList`
stores information on a request, that can be viewed on the Request UI panel. Select simply retrieves the information of
the request stored at that particular index of this list.
...
2. `RequestBookParser` will receive `assign` as the command and instantiate `AssignRequestParser` to further parse the command.
...
* Alternative 1 (current choice): The `Request` holds only some of the details of the healthworker assigned to it.
** Pros: Simplification of implementation.
** Pros: Does not violate the Law of Demeter (LOD), as the Request should only have limited knowledge of another object (`HealthWorker`, in this case).
** Cons: Makes it difficult to implement certain features, like obtaining the requests assigned to a particular healthworker, for example.

* Alternative 2: The `Request` does holds the details of the `HealthWorker` and the `HealthWorker` keeps track of a list of `Request`s.
...
** Cons: Violates the LOD principle, as the `Request` object should contain limited knowledge of the `HealthWorker` object.
** Cons: Complicates the code base, making it difficult to maintain in the long run.
...
=== Filter Feature
The `filter` command allows users to filter either the current displayed
list of `HealthWorker` and `Request` objects and replaces the current list
shown on the UI with the respective filtered lists.
...
test files. Developers are free to modify the code within the classes to generate person/patient/healthworker/request
book containing the respective data they require for tests. +
All the test file generator classes have a main function and hence can be ran independently by compiling the file.
...
** Think about how you can use SLAP to design the method. Where should we place the main logic of deleting conditions?
...
. Use different colors for different conditions inside person cards. For example, `friends` conditions can be all in brown, and `colleagues` conditions can be all in yellow.
+
**Before**
+
...
* handles many requests daily to support homecare services
* prefer desktop apps over other types
* can type fast
* prefers typing over mouse input
* is reasonably comfortable in using CLI apps

*Value Proposition*: helps eliminate the manual and messy workload of handling the different requests by integrating into a single management system.
...
|`* * *` |user |check the details of new or unassigned requests|see which requests have yet to be assigned a health worker and what type of request it is
|`* * *` |user |assign a health worker to an existing request|close an open request by assigning a health worker who has the skills to attend to the patient's needs at the time of the request

|`* * *` |user |register a new health worker|
|`* * *` |user |register a new request (either from the patient or caretaker)|key in a new request with the specific request and patient details
|`* * *` |user |filter requests by patient's conditions|perform assignment of staff to requests that fit their skillset
|`* * *` |user |filter requests by patient's name|find all the past requests he had with the system, if any.
|`* * *` |user |delete a health worker |remove health workers who are no longer active
|`* * *` |user |find a health worker by name |find a particular health worker registered in the system
|`* * *` |user |check the professional details of existing health workers|identify which health worker has the appropriate skillsets and certification to fufill the corresponding requests
...
|`* *` |user |set a reminder|phone up the patient to remind him that a health worker will be visiting his home on the day of the request
|`* *` |user |undo my previous action|I can revert some unintended actions in the system
...
[none]
* 1.a. Patient is not in Application:
[none]
** 1.a1. User registers the patient and the relevant details into the system.

** 1.a2. Application stores the data of newly registered patient
[none]
...
.. Double-click the jar file +
   Expected output: Shows the GUI with a set of sample requests and health workers. The
