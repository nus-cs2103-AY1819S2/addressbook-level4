...
[cols=",,,,,", options="header",]
|===
6+^|List of valid prefixes

6+^|Unique Prefixes
2+| *Patient* 2+| *Record* 2+| *Next-of-Kin*
|Prefix | Attribute | Prefix | Attribute | Prefix | Attribute
|n/ | Name | pro/ | Procedure | nokn/ | NOK Name
|sex/ | Sex | desc/ | Description | nokp/ | NOK Phone
|ic/ | Nric | | | nokr/ | NOK Relation
|dob/ | Date of Birth | | | noka/ | NOK Address
|p/ | Phone | | | |
|e/ | Email | | | |
|a/ | Address | | | |
...
The goto command specifies a patient by index.
It then brings the user to the records mode, where all dental records of the specified patient are listed.

Record commands can only be executed in this mode.
...
** `<<back, back>>` - Exits the record edit mode, and returns to the patient list.
