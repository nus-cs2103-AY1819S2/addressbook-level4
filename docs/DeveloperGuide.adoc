...
1. `CourseRequirement` - an interface that represents course requirement data in model.
2. `RequirementStatus` - an association class that links `CourseRequirement` with a list of `ModuleInfoCode` from `ModulesTakenList`. Each `RequirementStatus` class stores a copy of `ModuleInfoCode` from `ModulesTakenList`,
a `CourseRequirement`, as well as each of the results when functions of `CourseRequirement` acts on the list of `ModuleInfoCode`.


`CourseRequirement` interface follows a `Composite` design pattern. It is implemented by 2 classes,
`PrimitiveRequirement` and `CompositeRequirement`. A `Composite` design pattern is favoured as it allows us to treat
composition of `CourseRequirement` objects uniformly. This is achieved through the use of polymorphism.

`PrimitiveRequirement` stores a list of `Condition` objects and each `Condition` stores a
`Pattern` and an `int`, `minToSatisfy`. A `Condition` is satisfied if there are at least `minToSatisfy` distinct
`ModuleInfoCode` that matches `Pattern` in `Condition`.

`CompositeRequirement` contains two `CourseRequirement` interface objects as well as a `LogicalConnector` enumeration
to combine two different `CourseRequirement` using logical operations.
...
`ModuleInfoCode` satisfies

The results of above the 3 method is also stored in `RequirementStatus`.
...
* **Current choice: Checking Requirement fulfilled by only using `ModuleInfoCode` of
`ModuleTaken`**
...
and is sufficient to cover most cases. It is also more feasible to implement due to the limited amount of time we have.
