...
* Contains various helper method to access the `assets` directory, the storage for the raw image files.
...
[[sequencediagram_open]]
.Sequence Diagram for Open Command +
image::OpenCommandSequenceDiagram.png[width=800]

Given below is an example usage scenario of how `open` behaves at each step as shown in <<sequencediagram_open>>.
...
Given below is an example usage scenario of how `listfiles` behaves at each step as shown in <<sequencediagram_listfiles>>.
...
===== Design Considerations
 ...
 - Images that are still being edited have to be stored and edited in a temp folder first due to our `undo/redo` implementation.
...
[[sequencediagram_save]]
.Sequence Diagram for Save Command
image::SaveCommandSequenceDiagram.png[width=800]

Given below is an example usage scenario of how `save` behaves at each step as shown in <<sequencediagram_save>>.
