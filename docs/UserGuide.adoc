...
[NOTE]
====
This program requires a minimum resolution of 1000*810. Best viewed in full screen.
====
+
...
`label 2`. +
This selects the 2nd medicine in the inventory and output the information to a default PDF file named `to_print`.
...
- Below is a mock up on the UI.

. Analytical features to help predict when and how much the pharmacist have to order a new batch of medicine.
- Meditabs will analyse the history of individual medicine, such as how much quantity is dispensed and what frequency you reorder the medicine.
- This will help further streamline the management and allow you to focus on treating on your patients.
