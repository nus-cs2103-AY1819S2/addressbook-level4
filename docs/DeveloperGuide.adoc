...
from `Model` via `Model#getFilteredRestaurantList()`. The categories of the target restaurant is merged by calling
...
I chose Alternative 1 because there are not much categories a restaurant can have. By having one command to set any category,
