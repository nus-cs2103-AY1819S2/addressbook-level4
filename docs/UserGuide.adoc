...
* The count field is optional. It represents the number of cards the user wants in the session. By default, it sets to be 1.
* The number of cards should be at least 1. The start command will fail with count to be 0.
* The mode field only supports four mode: `learn, preview, review and difficult`.
. LEARN:
. PREVIEW:
. REVIEW:
. DIFFICULT:
...
* `start n/country-capitals c/15 m/LEARN`
...
Closes and saves a lesson after editing it. +
Format: `closeLesson INDEX`

Usage rules:

* Closes the lesson at the specified `INDEX`.
* To find the `INDEX` number of a lesson, type `listLessons` to list all lessons loaded in memory.

Examples:

* `closeLesson 1` +
Closes the first lesson in the numbered list displayed by `listLessons`.
