...
This segment will explain the key features of the a new data type `Task` that TeethHub handle. Key implementation details and the design considerations when designing these features will also be covered.

==== Current Implementation: Task storage and attributes

The `Task` class is a new type of object that is meant to represent any task a dentist would have to perform. `Task` objects are kept in a list and handled by the `UniqueTaskList` class. This follows the same process as the `Patient` class and by extension, the `Person` class which has a `UniquePersonList` handling a list of `Person` objects. Storing information of all objects does not change between `Task` and `Person` classes and both are stored into a `TeethHub.json` file. Attributes for `Task` are instantiated as objects with their own methods to validate the user's input much like the current implementation of `Patient`/`Person` .
...
==== Current Implementation: Task interaction feature (Basic)

Essential commands for basic interaction with the tasks such as, `taskadd`, `taskedit` and `taskdelete` are implemented in a similar fashion as compared to existing commands. All three commands extend from the abstract class `Command` and follow the same execution flow as `patientadd`, `patientedit` and `patientdelete`. The typical execution flow of these commands mostly involve the `Logic` and `Model` component of the application and the order can be detailed as follows:
...
The following sequence diagram illustrates the generic command execution flow and the components involved:

image::GenericSequenceDiagram.png[width=""]

==== Current implementation: Task Autocomplete feature

The `taskdone` command is a feature implemented to allow the user to automatically complete a task of his/her choice, setting it to `COMPLETED`. The command also allows the user to automatically add a record to the task's `LinkedPatient` upon completion of the task. This feature makes use of the `Model` by calling its API to perform additional interactions with both task and patient.
...
The following sequence diagram illustrates flow in which the command's execution takes when completing a task with the index `1`:

image::TaskDoneSequenceDiagram.png[width=""]

==== Current implementation: Task Calendar feature

The `taskcal` command is a feature implemented to allow the user to view task related data in a more interactive and helpful manner. Executing the command creates a popup window with the list of tasks on the left side and a calendar on the rigth side. This feature is implemented using calls to `JavaFX` with the most important component being `DatePicker` which allows a calendar to be drawn up and displayed to the user. This feature also enables the user to find tasks depending on the `date` they provide the command or the date cell they click on. Implementation of this aspect of the feature is done on the `UI` component of the application under the `CalendarWindow` class which can be found in the `ui` folder.
...
** **Choice Justification: Having an extremely large file is a very unlikely scenario and even in the case where it does happen, most operating systems these days utilize a filesystem that allows for filesizes far beyond 4GB. Compared to the cons of the alternative where performance and increased likelihood of issues in the futre, the pros that the alternative offers do not seem valid enough to justify using it over the current choice.

* **Alternative 1 (current choice):** Store the tasks in the same json file
** Pros: Reduces time spent opening a seperate json file. This results in less I/O work for application and thus an improvement in the application's load time. Easy to implement.
** Cons: The single json file that stores the data may become too large if there are too many patients and/or tasks. The size might exceed the limit of certain file systems like FAT-32 with a limit of 4GB, leading to the program unable to save to the file.

* **Alternative 2:** Store the tasks in a seperate json file
** Pros: Very unlikely to have a very large json file by saving different data types to a different file.
** Cons: Complicated to implement and affects future storage and I/O features. Implementation would affect I/O features that may need to read from the files to export both patients and tasks. Sets a precedent where new data types added in the future should be stored seperately for consistency, which would affect `export` and `import` commands as they have to be changed to read a new file to export/import.

===== Aspect: Representing the Priority class attribute for Task

* **Alternative 1 (current choice):** Use Java's special Enum class for Priority
** Pros: Easy to validate user input for the attribute. Adding new constants to the Enum class is easy to do and will not affect the code much. Enables the `taskdone` feature to change it for the user automatically. Good programming practice to use enum for values that should not be changed.
** Cons: Limits user's freedom when naming priority. Users have to name priorities as either `high`, `med` or `low`. Some users might prefer other naming conventions.

* **Alternative 2:** Use a normal class type, with string as inner attribute for Priority.
** Pros: Easy to implement as it functions similarly to how `name` and `title` works. Priorities can be named anything the user desires.
** Cons: Priority tracking depends on the user's own memory of what he/she named them and sees as a higher priority. Users may opt to use many levels of priority tracking which causes implementation of new features to interact with the priorities would be difficult as it requires tracking too many different things.

==== Aspect: Modifying task and patient when executing `taskdone` command

* **Alternative 1 (current choice):** Replace the task and patient with new instances with priority changed and record added respectively.
** Pros: Allows the task completion process to be undo-able and redo-able based on the current implementation of undo/redo.  
** Cons: Increases number of steps to perform a relatively simple operation on the stored data.

* **Alternative 2:** Modify the target task and linked patient directly
** Pros: Easy to implement as they already have methods to perform the operations. Reduces run time of taskdone feature.
** Cons: Completing a task does not work with undo and redo due to the way the model is implemented. Implementing changes model to support this would lead to too much work and could potentially introduce bugs for other commands.

==== Aspect: Data strcuture to support Task Calendar generation

* **Alternative 1 (current choice):** Use a HashTable to store dates that have task's starting and/or ending.
** Pros: Finding duplicate dates and replacing the stored priority number becomes fast and efficient when using a HashTable O(1) run time. Removing
** Cons: Increases number of steps to perform a relatively simple operation on the stored data.
