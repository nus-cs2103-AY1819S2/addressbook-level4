...
may wrongly detect that the application is not responding, when it infact is. Hence, should you see the "application not responding"
pop-up from your operating system, it is actually still working in the background therefore please allow the application some time
...
[.big]#Format: `history` [To Set a Deadline]# +
...
[TIP]
If you previous command was `encrypt` or `decrypt`, the commands will not be saved to the
