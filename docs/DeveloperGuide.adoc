...
The `UI` component is supported to a `Notifier` class. The class implements Java's `propertyChangeListener` as means of drawing updates to the User Interface.
...
* Validation check to see if raw image is a valid Image.
...
** Cons: Importing multiple images from same folder needs more commands.
...
** Cons: Can result in errors in too many images or images are of alternative formats.
// end::import[]
...
//tag::WaterMark[]
=== WaterMark Feature
This command allows the user to add a watermark to their image. Upon receiving an input message by the user, the message will be checked for its validity and will throw an error if it is empty or contains only spaces. Messages can contain spaces in between the words or numbers. The command will then be added to the `List<Commands>` belonging to `tempImage` found in `currentEdit` which saves the editing history of the targetted image.
 ...
 * `CurrentEdit#addCommand(Command)` - add this command to the edit history [`List<Command>`] in `tempImage` for the undo/redo function

 * `CurrenEdit#displayTempImage()` - Displays the temporary image stored in directory: "src/main/resources/temp/".

 * `Image#hasWaterMark()` - Checks if the `Image` already has a watermark.

 * `Image#setWaterMark(boolean)` - Sets the `Image` object's `hasWaterMark` field accordingly, if the `Image` has or does not have a watermark.
