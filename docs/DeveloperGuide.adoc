...
* users are not supposed to add data files into the app manually. If the user accidentally corrupt the data file, he or she may have to reinstall the app and all data will get lost.
...
This would be handy when there are many books in the book shelf. It also makes the app more user-friendly as the user sometimes cannot remember the exact full name of a book but only know a few things about the book desired.

Compared to to the `list` command in the address book app, our `listBookCommand` is an implementation of the combination of `list` and `find` and even more functionally helpful, as it accepts more filtering criteria than the original command does.
...
* **Alternative 1 (current choice):** Only books match all criteria provided will be selected. For example, if the user give 2 book name keywords and 2 ratings, only books matching at least 1 name keyword and at least 1 rating will be selected. Books match 1 name keyword but no ratings will not be considered.
** Pros: Easy to test and manage.
** Cons: Such searching technique may fail to provide the user with enough inforamtion.
* **Alternative 2:** Books that match at least one criteria will be selected. For example, if the user give 2 book name keywords and 2 ratings, books that match 1 name keyword but no ratings will still be selected.
** Pros: More likely to provide the user with the book that the user is looking for.
** Cons: Hard to test and manage. Such design also fails to narrow down search results when the user is providing more keywords for the targetd book.
...
Step 5. The `SummaryCommand` executes. The command will traverse all books in the book shelf, checks if there are authors or tags appear more than once and give relative details. The command will also show the highest rating of books in the book shelf.
...
** Cons: More methods are introduced into the model manager. To get include more information in the summary, not ony the summary command should be modified, but also more information query methodS should be added in THE `Model`.
...
* ReviewMessage should be less than or equal to 400 characters. This is because the
BookBrowserPanel used to display the ReviewMessage currently cannot hold more than 400 characters.
Longer text also makes it more difficult to edit in the small CommandBox. Future releases would consider
...
Selection of review is implemented the same way as selection of book.

In addition, review message of selected review will be displayed in the book browser panel.
...
Step 4. The BookBrowserPanel listens to the change in selected review, and displays its review message.


==== DeleteReview Feature
===== Current Implementation

Review is deleted based on index.
...
.. After the installation, `bookshelf.json` is not present. The file is initialized after main app runs for the first time. At this point the file is loaded with the sample data.
