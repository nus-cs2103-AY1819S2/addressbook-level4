...
[source,java]
----
include::../src/main/java/seedu/address/model/MapGrid.java[lines=38..45]
----
...
[source,java]
----
include::../src/main/java/seedu/address/model/MapGrid.java[lines=92..98]
----
...
A `Battleship` is a class representing a ship that can be placed on the map grid. A `FleetEntry` class is internally used to store the `Battleship` in a player's `Fleet`. `FleetEntry` is a nested class within `Fleet` that contains a `Battleship`, its head `Coordinates` and its `Orientation` on the map grid.

The length of the ship varies depending on the type of ship. The user can tag a battleship using a `t/[TAG]` parameter in the `put` command. Tagging a ship is optional. +
...
The head coordinate of a ship refers to the top-most and left-most cell of the ship. When coordinates are specified in the `put` command, it specifies the coordinates of the head of the ship, as shown in the figure below.

.The coordinates of the battleship head are always the top-most and left-most coordinates.
...
The boundary checks are essential to the functionality of both the human player and the computer player. This is because `BoundaryValueChecker` is used by the computer player when it is deploying its own ships after the human player has ended his or her turn. `BoundaryValueChecker` relies on four separate methods to perform its checks. They are:
...
1. `isBattleshipAbsent()` - Checks if there are no _other_ ships that are situated on the head coordinates.
1. `isClear()` - Checks if there are no _other_ ships that situated along the body of the ship.

These methods check both the horizontal and vertical boundaries when a ship is being deployed. They also distinguish between the head and body of a ship in order to provide more specific feedback for the user. A code snippet is provided to illustrate an example of one such check, `isHeadWithinBounds()`.

[source,java]
----
include::../src/main/java/seedu/address/model/BoundaryValueChecker.java[lines=78..85]
----

The following sequence diagram illustrates what happens when the `put` command is called.
...
[source,java]
----
include::../src/main/java/seedu/address/model/Player/Fleet.java[lines=170..179]
----
...
|List|Example
|All ships|`list`
|Ships with certain tags| `list t/tag1 t/tag2`
|Certain ships| `list n/destroyer n/cruiser`
|Certain ships with certain tags| `list n/destroyer n/cruiser t/tag1 t/tag2`
|====================
...
The `list` command can list certain ships with certain tags by taking the filtering the list of deployed ships. The following sequence diagram shows how deployed battleships are listed when the `list` command is entered into the command line.

.Sequence diagram of listing battleships deployed on the map.
image::ListShipSequenceDiagram.png[]

The use case diagram below illustrates the design considerations that have gone into the design of the `list` command.
...
The above figure is a high-level overview of what happens when the
user does battle against the computer enemy. As shown above, the player
...
. Should not use up too much memory for the application and application data for typical usage.
. Versions should be backward compatible with each other.
. Should be usable by a novice who has never played Battleship before.
