...
public class VersionedAddressBookTest {
    ...
    private final ReadOnlyBookShelf addressBookWithAmy = new AddressBookBuilder().withPerson(AMY).build();
    private final ReadOnlyBookShelf addressBookWithBob = new AddressBookBuilder().withPerson(BOB).build();
    private final ReadOnlyBookShelf addressBookWithCarl = new AddressBookBuilder().withPerson(CARL).build();
    private final ReadOnlyBookShelf emptyAddressBook = new AddressBookBuilder().build();

    @Test
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedBookShelf versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedBookShelf versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedBookShelf.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedBookShelf.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedBookShelf versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedBookShelf copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedBookShelf differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedBookShelf differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedBookShelf versionedAddressBook,
        ...
        assertEquals(new BookShelf(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlyBookShelf expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new BookShelf(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlyBookShelf expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new BookShelf(versionedAddressBook));
        }
    ...
    /**
     * Creates and returns a {@code VersionedBookShelf} with the {@code addressBookStates} added into it, and the
     * {@code VersionedBookShelf#currentStatePointer} at the end of list.
     */
    private VersionedBookShelf prepareAddressBookList(ReadOnlyBookShelf... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedBookShelf versionedAddressBook = new VersionedBookShelf(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedBookShelf versionedAddressBook, int count) {
