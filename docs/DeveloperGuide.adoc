...
The following sequence diagram summarizes what happens when a user executes the Consultation command:
...
of months, Logic will call the ModelManager's getStatistics(topic, FROM_YEARMONTH, TO_YEARMONTH), which then calls
...
Statistics object. The Statistics object will then be converted into a FinanceStatistics or ConsultationStatistics
depending on the topic queried. RecordManager will then return the FinanceStatistics/ConsultationStatistics back to
...
The statistics command is started through the command `stats TOPIC START_MMYY [END_MMYY]`.
Current available topics are: `finances`, `consultation`, and `all`. Future topics: `symptoms` and `medicine`
will be added in future milestone v1.3. The two MMYY corresponds to a range of dates. The end range is optional,
