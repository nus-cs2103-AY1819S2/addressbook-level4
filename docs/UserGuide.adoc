...
This command will allow users to find more about a module or even compare modules so as to decide which modules to read
...
.Multiple module codes serach command format
...
who find themselves in such a situation ca search for modules by their names by adding a `n/` prefix after the
