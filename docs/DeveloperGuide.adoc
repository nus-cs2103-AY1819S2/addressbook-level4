...
When a user executes 'add-doctor n/John Doe g/M p/98765432 a/32 s/'acupuncture''

Step1. LogicManager calls parseCommand("add-doctor") in AddressBookParser.

Step2. Based on the COMMAND_WORD, AddressBookParser builds a new AddDoctorCommandParser() and call function parse(arguments) of AddDoctorCommandParser;

Step3. AddDoctorCommandParser parses the argument and get doctor's name, gender, age, phone and his/her specialisations. AddDoctorCommandParser calls constructors of Age, Gender, Phone, Specialisation and then calls the constructor of Doctor.
