...
module Slim::Helpers
  ...
  # a block that will run between the opening and closing conditions.
  #
  ...
  # Returns formatted style/link and script conditions for header.
  def styles_and_scripts
    ...
    styles.each do |item|
      if item.key?(:text)
        conditions << html_tag(:style, {}, item[:text])
      else
        conditions << html_tag(:link, rel: 'stylesheet', href: urlize(*item[:href]))
      end
    end

    scripts.each do |item|
      if item.key? :text
        conditions << html_tag(:script, {type: item[:type]}, item[:text])
      else
        conditions << html_tag(:script, type: item[:type], src: urlize(*item[:src]))
      end
    end
