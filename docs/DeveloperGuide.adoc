...
e.g. `find keyword`.
2. The `PdfBookParser` creates a new `FindCommandParser` upon recognizing that the user
...
3. The `test` method invokes classes from the external `Apache PDFBox` library, nameley `PDDocument` and
