...
* <<Design-QuizModel,*`QuizModel`*>>: Holds the data of the Quiz in-memory.
...
.  `Logic` uses either the `BrainTrainParser` or `QuizModeParser` class to parse the user command.
..  If `BrainTrainParser` is used to parse the user command:
...
*API* : link:{repoURL}/src/main/java/seedu/address/managementModel/Model.java[`Model.java`]
...
*API* : link:{repoURL}/src/main/java/seedu/address/quiz/QuizModel.java[`QuizModel.java`]

The `QuizModel`,

* stores a `Quiz` object that represents the list of `QuizCard`.
* stores the Address Book data.
* does not depend on any of the other four components.
...
The data storage feature manages the saving of data, such as lessonList and user progress, to disk for the application. This lets the user keep their progress between usage sessions.
...
* Parsing data from file into lessonList
...
Lessons are parsed by CsvLessonsStorage, being converted between Lesson and List<String[]> formats. The List<String[]> format is the primary format handled by CsvUtil for reading and writing to CSV files.

For lessonList, there are three main components when saved to a CSV file.
...
===== Header
The header stores the set up information for the lesson. For more information on the format of lessonList, please read `TODO`
...
==== Current Implementation
The quiz feature implements the following commands and processes answer:
...
==== \status
The `\status` feature list the total attempts you have attempted, total number of question you
have gotten correct in this session and which question you are currently on.
...
** Pros: Answer can be anything excluding word starting with a backslash `\` (e.g. `\help`)
** Cons: Tedious to implement
* **Alternative 2:** Run quiz in the same mode as the rest of the management commands
** Pros: Easier to implement compared to alternative 1
** Cons: Answer cannot start with those from the list of management commands (e.g. `start`)
// end::quiz[]
...
*Scenario:* You are in charge of `managementModel`. One day, the `logic`-in-charge approaches you for help. He wants to implement a command such that the user is able to remove a particular tag from everyone in the address book, but the managementModel API does not support such a functionality at the moment. Your job is to implement an API method, so that your teammate can use your API to implement his command.
...
****
* Hints
** The link:{repoURL}/src/main/java/seedu/address/managementModel/Model.java[`Model`] and the link:{repoURL}/src/main/java/seedu/address/managementModel/AddressBook.java[`AddressBook`] API need to be updated.
...
* Solution
** Implement a `removeTag(Tag)` method in link:{repoURL}/src/main/java/seedu/address/managementModel/AddressBook.java[`AddressBook`]. Loop through each person, and remove the `tag` from each person.
** Add a new API method `deleteTag(Tag)` in link:{repoURL}/src/main/java/seedu/address/managementModel/ModelManager.java[`ModelManager`]. Your link:{repoURL}/src/main/java/seedu/address/managementModel/ModelManager.java[`ModelManager`] should call `AddressBook#removeTag(Tag)`.
...
* able to keep track of the progress of all <<lesson,lessonList>>
...
|`* * *` |user |load/import lessonList |I can use the App without needing to make my own lists first

|`* * *` |Creator |export lessonList |I can share my lessonList easily in a multiple files
...
|`* * *` |user |start multiple lessonList |I can see all the question at once
...
|`* * *` |user |add tags to the lessonList |I can search based on the tags
...
|`* *` |user |view lessonList created by others |I can import them into my own library without creating my own
...
|`*` |user |autocomplete the lessonList name |I can find a lesson quickly

|`*` |Creator |import from other file formats into lessonList |I can use my own pre-existing lists with the application
...
[discrete]
=== Use case: Start multiple lessonList

*MSS*

1. User finds specified lessonList.
2. User starts lessonList by lesson names.
3. App displays questions for User to answer.
4. User to input his answer.
5. App displays the correct answer.
+
...
.  Should be able to hold up to 1000 lessonList without a noticeable sluggishness in performance for typical usage.
