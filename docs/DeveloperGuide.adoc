...
The 'listUnvisited' feature accepts the user's current postal code and displays unreviewed restaurants nearest to the user's postal code.
This mechanism is facilitated by `PostalDataSet`.
It contains a `HashMap<Integer,PostalData> of `PostalData` which contains the x and y-coordinate of every postal code in Singapore.

`PostalDataSet` is constructed by `JsonPostalDataStorage` which processes the `data.json` file from `JsonSerializablePostalData` into `PostalDataSet`.
This is exposed in the `Storage` interface as `Storage#loadPostalData()`
...
Step 2. The user calls `listUnvisited postal/267951`. This command will call the method `Model#filterAndSortByLocation(PREDICATE_SHOW_UNVISITED_RESTAURANTS, postal)`

Step 3. The `Model#filterAndSortByLocation()` will first filter the `RestaurantList` to show all `Restaurants` with zero reviews. It will then call `PostalDataSet#checkPostal(int postal)` for each of the restaurants then sort it based on the distance from the given postal code.

Step 4. The `filteredList` will then be updated to show the nearest `Restaurants` with zero review.
...
The restaurants are categorised by a `Categories` field in the `Restaurant` class.
The `Categories` class encapsulates all the different categories a restaurant can have such as `Cuisine` and `Occasion`.
Additionally, it implements the `merge` operation. The `merge` operation is essential in ensuring that when one categories
...
categories: `cuisine` and `occasion`, doing a `setCategories` for `cuisine` will override its `occasion` categories if the
merge operation is not implemented.

The `merge` operation is implemented by taking in two `Categories` arguments, `previous` and `updated`. All categories in
`updated` will replace any existing categories in `previous`. The merged `Categories` will retain those categories in
`previous` for any categories not present in `updated`.

The `merge` operation is used by `SetCategoriesCommand#execute()` to update a restaurant's categories by merging it with
its existing categories.

The `setCategories` command is implemented as follows:

* `SetCategoriesCommandParser` parses the user input arguments, creating a `SetCategoriesCommand` object with the target `Index`
and new `Categories` object containing all the categories in the user input.
* Inside `SetCategoriesCommand#execute()`, the restaurant identified by the index is retrieved from `Model`.
* Existing categories are retrieved from the target restaurant.
* New `Categories` object is created by merging existing categories and categories inputted by the user.
* The new `Categories` object is used to create a new `Restaurant` with the updated `Categories` object.
...
1. FoodLover enters command, categories to filter by (i.e. cuisine, occasion, price range) and keyword / price range (e.g.japanese, lunch, 5 10)
...
[none]
* 1a. FoodLover enters an invalid categories
[none]
** 1a1. FoodLover tells user that no such categories exists.
+
Use case ends.
...
1. FoodLover enters command, categories to sort by (i.e. rating, price)
2. FoodDiary displays restaurants according to categories
+
Use case ends.
