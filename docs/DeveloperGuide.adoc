...
===== [Step 6] Storage: Add `Remark` field to `JsonAdaptedRestaurant` class
We now have `Remark` s for `Restaurant` s, but they will be gone when we exit the application. Let's modify link:{repoURL}/src/main/java/seedu/address/storage/JsonAdaptedRestaurant.java[`JsonAdaptedRestaurant`] to include a `Remark` field so that it will be saved.
...
. Fix `invalidAndValidRestaurantAddressBook.json`, `typicalRestaurantsAddressBook.json`, `validAddressBook.json` etc., such that the JSON tests will not fail due to a missing `remark` field.
...
|`* * *` |user |add a new restaurant |

|`* * *` |user |delete a restaurant |remove entries that I no longer need

|`* * *` |user |find a restaurant by name |locate details of restaurants without having to go through the entire list

|`* *` |user |hide <<private-contact-detail,private contact details>> by default |minimize chance of someone else seeing them by accident

|`*` |user with many restaurants in the address book |sort restaurants by name |locate a restaurant easily
|=======================================================================
...
2.  AddressBook shows a list of restaurants
3.  User requests to delete a specific restaurant in the list
4.  AddressBook deletes the restaurant
+
Use case ends.
