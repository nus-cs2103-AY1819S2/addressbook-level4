...
public class VersionedDocXTest {
    ...
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedDocX versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedDocX versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedDocX.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedDocX.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedDocX versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedDocX copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedDocX differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedDocX differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedDocX versionedAddressBook,
                                             List<ReadOnlyDocX> expectedStatesBeforePointer,
                                             ReadOnlyDocX expectedCurrentState,
                                             List<ReadOnlyDocX> expectedStatesAfterPointer) {
        // check state currently pointing at is correct
        assertEquals(new DocX(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlyDocX expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new DocX(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlyDocX expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new DocX(versionedAddressBook));
        }
    ...
    /**
     * Creates and returns a {@code VersionedDocX} with the {@code addressBookStates} added into it, and the
     * {@code VersionedDocX#currentStatePointer} at the end of list.
     */
    private VersionedDocX prepareAddressBookList(ReadOnlyDocX... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedDocX versionedAddressBook = new VersionedDocX(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedDocX versionedAddressBook, int count) {
