...
The interactions between all four components when a user executes the `deleteFromMenu` command is very similar to the
 interactions when <<AddToMenu, adding to menu>>, as shown by the sequence diagram below:

.Component interactions for `deleteFromMenu` command
image::DeleteFromMenuSequenceDiagram.png[width="800"]
...
After parsing the input text string by the [.java]#RestOrRantParser#parse()# method, the [
.java]#DeleteFromMenuCommand# object is created.

[.java]#DeleteFromMenuCommand# then makes use of the method [.java]#Menu#getItemFromCode()# to retrieve the menu item
 from the item [.java]#Code# input. The code snippet of the method is shown below:
...
The interaction between the components for this function is similar to <<ClearOrder, clearing an order>>, a small
difference being `clearMenu` clears all menu items, while `clearOrder` only clears orders within a specific table.

When `clearMenu` is keyed in, the command is parsed by [.java]#RestOrRantParser# and a [.java]#ClearMenuCommand#
object is returned by the parser.

When [.java]#ClearMenuCommand#execute()# is called, it will check if the restaurant is occupied, and if the menu is
empty. [.java]#CommandException# with respective error messages will be thrown if either of the conditions are true.
After ensuring that the restaurant is unoccupied and the menu is non-empty, the menu will be cleared by initializing
the menu with an empty ArrayList, with the [.java]#Model#setMenuItems() method.
...
1.  User enters the item's code, name and price.
2.  RestOrRant updates the list of menu items in the side panel to include the added menu item.
+
Use case ends.
