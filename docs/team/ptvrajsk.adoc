...
`model` components as a menial version of it was already partly implemented.
...
As for the Find Feature, there were several aspects that had to be communicated
to the end user namely, the number of files that were detected by the `find`
command as well as the careful use of third-party libraries. +
The relevant pull requests pertaining to this feature can be
referenced in the following _Github_ issue
...
6. The `model` then updates itself and stores the current action in the `history`.
...
** Increase ease of use for less technical users.
...
revamping old TestCases from previous projects to suite new project direction. link:https://github.com/CS2103-AY1819S2-T12-4/main/pull/281[#218]
* Proof-Reading and Enhancing of the Entire User-Guide Document. link:https://github.com/CS2103-AY1819S2-T12-4/main/pull/313[#313]
