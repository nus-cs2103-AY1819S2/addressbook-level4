...
[NOTE]
As a more OOP model, we can store a `Hint` list in `VersionedCardFolder`, which `Card` can reference. This would allow `VersionedCardFolder` to only require one `Hint` object per unique `Hint`, instead of each `Card` needing their own `Hint` object. An example of how such a model may look like is given below. +
 +
