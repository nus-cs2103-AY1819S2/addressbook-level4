...
image::ModelComponentWithActivityList_copy.png[width="600"]
...
image::ActivityAddChecks.png[width="500", height="300"]
...
public boolean test(Activity activity) {
    ...
    if (keywords.containsKey(PREFIX_ALL)) {
        return keywords.get(PREFIX_ALL).stream()
            .anyMatch(...); //String util to check if keywords in ActivityName or ActivityDescription
    }                       // or ActivityLocation
   ...
   boolean isDescriptionMatched = ... //checking if keywords in description

   boolean isLocationMatched =  ... // checking if keywords in location
