...
4. Appointments and Reminders are displayed on the `ReminderListPanel`. Appointments are coloured blue, medicine alarms are coloured red
...
===== Adding appointment into AppointmentManager
...
[NOTE]
If the user does not provide any parameters together with `listapp`, the appointments for the current week will be listed.

[[listapp]]
===== Listing by dates
The user can specify the range of dates by providing a `FORMAT` (`day`, `week`, or `month`) and a `DATE`, meaning to
...
. The `ListAppCommandParser` class parses the NRIC into an `NRIC` object, to create a `ListAppCommand` object with this
`NRIC` field.
...
[NOTE]
`Model#listApp()` and `AppointmentManager#listAppointments()` are overloaded methods. They have different method
signatures based on their parameters. One implementation takes in two `LocalDates`, `start` and `end`, while the other
implementation takes in a single `Patient` object. This allows the same method name to be called, and list appointments
by either providing a given range of dates or by providing a patient.
...
. The `DeleteAppCommandParser` class parses the user input into a `LocalDate date` and `LocalTime start` that specifies
...
by using the method `getAppointments()`.
...
Since these appointments are sorted by date and time, the earliest appointment is on the left and the latest on the right.
...
All slots created will be added into a list of free slots, `freeSlots`.
...
to generate a `String` that represents all the free slots to be printed on the main display of QuickDocs.
...
* The user is able to set a threshold for each medicine in the inventory, using the `alarm` command. When the quantity
...
. `ListRemCommand` will then be executed
...
Displaying a single reminder can also be done by clicking on the reminder at the sidebar with a mouse. This feature
...
. `DeleteRemCommand` will then be executed
* Firstly, it retrieves the currently displayed `List` of reminders by calling `Model#getFilteredReminderList()`.
* Next, it checks if the given `Index` is valid, checking if the `Index` is present in the current `List`
of reminders.
