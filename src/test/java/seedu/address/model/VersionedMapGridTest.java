...
public class VersionedMapGridTest {
    ...
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedMapGrid versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedMapGrid versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedMapGrid.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedMapGrid.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedMapGrid versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedMapGrid copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedMapGrid differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedMapGrid differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedMapGrid versionedAddressBook,
        ...
        assertEquals(new MapGrid(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlyAddressBook expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new MapGrid(versionedAddressBook));
            versionedAddressBook.redo();
        }
    ...
    /**
     * Creates and returns a {@code VersionedMapGrid} with the {@code addressBookStates} added into it, and the
     * {@code VersionedMapGrid#currentStatePointer} at the end of list.
     */
    private VersionedMapGrid prepareAddressBookList(ReadOnlyAddressBook... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedMapGrid versionedAddressBook = new VersionedMapGrid(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedMapGrid versionedAddressBook, int count) {
