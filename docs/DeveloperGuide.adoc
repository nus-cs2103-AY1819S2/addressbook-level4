...
image::SDforImagecommand.png[width="800"]
...
* `contrastCommand`: Applies the specified contrast ratio value on the targetted image
* `brightnessCommand`: Applies the specified brightness ratio value on the targetted image
* `black/whiteCommand`: Transforms colored targetted image to a black and white image

The following describes main operations and processes contained in the classes above.

==== ContrastCommand:

This command calls for an adjustment of contrast on a targetted image and adds this specific command called by the user to `Image`'s `List<EditCommands>` which saves the editing history of the targetted image.

Given below is an example usage scenario and how the command should behave at each step.

Step 1. When the users run the application and open an image to edit, a new `Image` object is created which stores the filename of the image,  history of edits [`List<EditCommand>`] and the image's metadata. This `Image` object will be set as the `currentImage` in `Model` for editing.

Step 2. When the user enters the command (e.g. `contrast 0.3`), the entered command is parsed and the command will be executed.

Step 3. During execution, the `execute` method in the `contrastCommand` class calls for the methods `Model#applyContrast()` and `Model#displayImage()`.

Step 4. Calling `Model#applyContrast()` creates a `BufferedOpFilter` object in the external library [`scrimage`] named `contrastFilter` which is applied on the image with the filepath: "src/main/resources/temp/" + `currentImage#getFileName()`

Step 5. Calling `Model#displayImage` updates the user interface and the user interface will now display the image with the filepath: "src/main/resources/temp/" + `currentImage#getFileName()`
...
image::ContrastCommandSequenceDiagram.png[width="740"]
...
* `Image#getHistory()` -- Returns commandHistory sublist using index to track which commands should be included.
...
Step 1. The user opens an image. The `Image` will be initialized with a commandHistory list containing a null command, and the index pointing to that command.
...
Step 3. The user wants to undo the previous transformation by using the `undo` command. It will call `Model#replaceTempImage()` which will replace the edited image in temp folder with the original image. It will then call `setUndo()` to set the index in `Image` and retrieve the list with `getHistory`. Using the list, it will apply the commands onto the original image till it reaches the command just before the previous transformation.
...
Step 4. After executing Undo, it will set a flag in `Image` to true. When the user executes another transformation, if the flag is set to true, it will delete all commands in the list at indexes greater than current index before adding the current command into the commandHistory
and setting flag back false.
...
[NOTE]
If the `Image#index` is 0, pointing to the initial photo, then there are no previous commands to restore. The `undo` command uses `Model#getImage()#canUndo()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the undo.
