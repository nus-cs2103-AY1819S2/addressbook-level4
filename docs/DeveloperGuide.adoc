...
==== \quit
The `\quit` will force quit the current quiz and save the attempted question to file. Once quit,
it will return back to the `ManagementMode`.
...
have gotten correct in this quiz and which question you are currently on.
...
** Pros: Answer can be anything excluding word starting with a backslash `\` (e.g. `\help`) and
have a better UX experience without having to type additional command as answer.
