...
* *`memberAdd n/John Doe mn/A0123456Z p/98765432 e/johnd@example.com a/311, Clementi Ave 2, #02-25 g/Male y/2 m/Chemistry t/Swimming`* : adds a member named `John Doe` to the ClubManager.
...
image::MemberCommands.png[width="1000"]
...
Only a valid matriculation number can be entered.| A0123456Z

|`Phone Number`| PHONE_NUMBER refers to the member's phone number.

Only 8 digits are accepted, starting with 6, 8 or 9. | 91234567
...
Only positive integers are accepted.| e.g. 1
|`Major`| MAJOR refers to the field of discipline in which the member is currently enrolled in.| Chemistry
...
* `memberAdd n/NAME mn/MATRIC_NUMBER p/PHONE_NUMBER e/EMAIL a/ADDRESS g/GENDER yos/YEAR_OF_STUDY m/MAJOR [t/TAG]...`
...
* `memberAdd n/Jane Lim mn/A0654321T p/91324756 e/janelim@example.com a/123, Sengkang Ave 6, #02-123 g/Female y/3  m/physics t/Swimming`
...
*(below will be implemented in 1.4)*
fimage::memberList_example_all.png[width="300"]
...
==== Edit an activity: `activityEdit` *up in v1.4*
...
* *Add a activity* : `activityAdd n/ACTIVITY_NAME d/DATE_TIME l/LOCATION [de/DESCRIPTION] [ic/IN_CHARGE_MATRIC] ...` +
e.g. `activityAdd n/Sentosa Outing d/16022019 1000 l/Harbour Front MRT de/Bring swim suits.`
* *Add a member to activity* : `activityAddMember ACTIVITY_INDEX MATRIC_NUMBER` +
