...
The `GoToCommand` is facilitated by the `Command` abstract class.
...
The current implementation to go back to the patient list uses the `back` command.

It is also facilitated by the `Command` abstract class.
...
All stored patients will be loaded and the user will be in the patient list by default.
...
The patient list is now replaced by the dental record list.

Step 3. The user can now add, edit, or delete dental records, which will be tied to the specified patient.
...
Step 4. The user now decides that he wants to go back to the patient list.
...
Step 5. The user can now add, edit, or delete patients or their personal information.
