...
Step 2. `WarningPanel#setUpVBox` is called to create two `WarningListView` so that the `ObservableList` can be displayed. Each list will be passed into one `WarningListView` as a parameter, along with a `String` descriptor of that list.
