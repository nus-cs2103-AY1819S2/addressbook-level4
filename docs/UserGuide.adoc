...
* Currently QuickDocs only support one filter per search.
* If more than one filter is entered (i.e. `listpat n/NAME r/NRIC`, only the first search criteria will be used
(in this case, name)

For example, if you are unsure of the patient's NRIC to start a command, you can always turn to `listpat` to help narrow down your search.
