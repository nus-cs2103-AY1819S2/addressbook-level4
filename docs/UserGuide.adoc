...
* *Table  Panel*: This panel gives you an overview of all tables in RestOrRant with their table numbers and respective occupancies.
...
[IMPORTANT]
Changing the table's occupancy to 0 will automatically clear all records of its orders which will not be included in RestOrRant's statistics.
...
a| a| For example, when all the customers have left the restaurant, RestOrRant will have all unoccupied tables as shown below.
