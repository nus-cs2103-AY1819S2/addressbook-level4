...
==== Current implementation - Wash a single indicated apparel
...
==== Current implementation - Wear a single indicated apparel
...
Sort the list of items based on the options given. The sorting will be increasing order of the name of the option specified.
Simulating an event in the real world where users arrange their wardrobe according to the category of their choice. For example, in a wardrobe where the folded shirts and pants are stacked on top of one another, the user might want to put all the shirts on top of all the pants.

==== Current Implementation
The sort command takes in one parameter. The valid arguments are: NAME, COLOR, and TYPE.

* Name  : The full name of the apparel.
* Color : The color of the apparel.
* Type  : The clothing type of the apparel.

e.g `sort color` will sort the apparels by color. If there are only 3 colors - Red, Black, Blue - in the wardrobe, calling `sort color` will arrange all the apparels in the following color starting from top - Black, Blue, Red.

.High Level Visualisation of Sort Command
image::HighLevelSortDiagram.png[width="800"]

When the user inputs the sort command in the CLI, the input is parsed, and the sorting option specified are validated.

`sort` keyword will call a constructor for SortCommandParser to parse the sort option, *color*, creating a SortCommand for LogicManager.

LogicManager executes SortCommand which calls Model to sort the apparels in the list by color, and commits the changes to Storage.

.Sequence Diagram for Sort command
image::SortCommandSequenceDiagram.png[width="800"]

The CommandResult object that is returned by the sort command lets the user know that the apparels have been sorted according to the specified option.

==== Design Considerations:

* Alternate command word: `sort name desc` works similar to `sort name` except that it sorts in descending order of the apparel's name.

Pros: Give the users more flexibility on how they want to sort the apparels.
Pros: Simulates the real world where people might to sort the apparels in reverse order.

==== Proposed additions for v2.0:
A sort command that is able to sort the apparels by multiple parameters. e.g. `sort color name type` to sort the apparels by type first, followed by name second, and lastly by color.

// end::sort[]

// tag::swap[]
=== Swap Apparel feature
Swap any two apparels.
Simulating an event in the real world where users swap the position of two apparels.

==== Current Implementation
The swap command takes in two parameters: INDEX_A and INDEX_B

* Index_A  : The first apparel to be swapped.
* Index_B : The second apparel to be swapped.

e.g `swap 2 5` will swap the apparels at index 2 and 5.

.High Level Visualisation of Sort Command
image::HighLevelSwapDiagram.png[width="800"]

When the user inputs the swap command in the CLI, the input is parsed, and both the indexes of the apparels specified are validated.

`swap` keyword will call a constructor for SwapCommandParser to parse the swap parameters, *1* and *2*, creating a SwapCommand for LogicManager.

LogicManager executes SwapCommand which calls Model to swap the position of apprels in index *1* and *2*, and commits the changes to Storage.

.Sequence Diagram for Swap command
image::SwapCommandSequenceDiagram.png[width="800"]

The CommandResult object that is returned by the swap command lets the user know that the position of the two apparels specified have been swapped.
...
==== Current Implementation
Below is a high level visualisation of the statistics command and a step-by-step explanation of an example scenario for the usage of `stats`.

.High Level Visualisation of Statistics Command
image::HighLevelStatsDiagram.png[width="800"]
...
.Sequence Diagram for Statistics command
image::StatisticsCommand.png[width="1000"]

The following is an activity diagram that sums up the Statistics command:

.Activity Diagram for Statistics command
image::StatisticsActivityDiagram.png[width="800"]

==== Design Considerations

* *Alternative 1 (current choice):* Go through all the apparels in Fashion Match and displays statistics whenever the statistics command is called.
** Pros: This approach is less intensive on the computational power of the user's PC, as the command is only executed whenever the user calls it.
** Cons: Less user friendly as statistics will not be real-time

* *Alternate 2:* Updates statistics in real-time and whenever the statistics command is used.
** Pros: This approach is more user-friendly as users would not need to type `stats` command after new apparel is changed.
** Cons: Time-consuming and not scalable. This method could be more computationally more intensive, especially if the user has a large wardrobe.

==== Proposed additions for v2.0

`Statistics` command will be a real-time feature that captures the state of the user's wardrobe and apparel in real-time and does not need to call the `Statistics` command each time. It will also occupy a dedicated space in the UI.
// end::stats[]
...
. Displays statistics of wardrobe

.. Prerequisites: Displays statistics using the `statistics` command.
.. Test case: `stats` +
   Expected: Details of the statistics of the wardrobe is shown in the status message. Timestamp in the status bar is updated.
