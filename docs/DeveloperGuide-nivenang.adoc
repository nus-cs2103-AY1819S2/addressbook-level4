...
image::SearchCommandClassDiagram.png[width="450", caption="Figure 4.2.1.1: ", reftext="Figure 4.2.1.1"]
...
image::SearchCommandSequenceDiagram.png[width="500", caption="Figure 4.2.1.2: ", reftext="Figure 4.2.1.2"]
...
image::SearchCommandActivityDiagram.png[width="450", caption="Figure 4.2.1.3: ", reftext="Figure 4.2.1.3"]

===== Search Name Feature
The command word for search name is `search` and is parsed by `TravelBuddyParser`.
The name arguments are then passed into `SearchCommandParser` and are stored in a list of keywords before being passed into `NameContainsKeywordsPredicate`. The list is converted into a stream and individually matched to the names of each entry in TravelBuddy.
...
===== Search Tags Feature
The command word for search tags is `searcht` and is parsed by `TravelBuddyParser`.
The tag arguments are then passed into `SearchTagsCommandParser` and are stored in a list of keywords before being passed into `TagContainsKeywordsPredicate`. The list is converted into a stream and individually matched to the tags of each entry in TravelBuddy.
...
// TODO: Update link for CountryCodes.adoc
[NOTE]
...
==== Current Implementation
// TODO: Add numbering in the diagram.
...
Step 4: `AddCommandParser`'s constructor is invoked.
...
// TODO: The app starts with some sample data already populated. Update screenshot accordingly.
...
117417 t/faculty` to add the place to TravelBuddy and presses kbd:[ENTER]. TravelBuddy will start executing the steps mentioned in Figure 10.
...
==== Design Considerations
// TODO: Learn from Chung Ming's portion. Change to table. Make sure to bold the starting words.
===== Aspect: Data structure to store Country Codes
* **Alternative 1 (current choice):** Use `enum` specified in `java.util.Locales`.
** Pros: Easy to implement and contains all three-letter country codes as specified in ISO-3166.
** Cons: Slightly slow in searching for country code.
* **Alternative 2:** Create a data structure containing the most commonly traveled countries in the world.
...
.  Constraints: e.g. the system should be backward compatible with data produced by earlier versions of the system;
.  system testers are available only during the last month of the project; the total project cost should not exceed $1
.5 million.
