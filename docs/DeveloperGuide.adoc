...
* users are not supposed to add data files into the app manually. If you accidentally do so and find the data corrupted, you should reinstall the app and all previous data will get lost.
...
The current version of listBook command works to help the user to search books that satisfy certain criteria. The user can look for books by giving keywords of book name, author. The user can also search book by tags or rating. This would be handy when there are many books in the book shelf. It also makes the app more user-friendly as the user sometimes cannot remember the exact full name of a book but only know a few things about the book he/she is looking for. For example, if the user is looking for a book with the keyword ‘Girl’ two or three years ago, and he only knows that he rated this book highly, then after he key in `listBook n/girl m/7 m/8 m/9 m/10` and press the enter, he will find the book he is looking for in the book list panel.  Or he cannot remember how to spell Garcia Marquez’s full name, he will be able to locate the book by using `listBookCommand t/magical`.

Compared to to the `list` command in the address book app, our `listBookCommand` is an implementation of the combination of `list` and `find` and even more functional helpful, as it accepts more filtering criteria than the original command does.
...
Step 3. The `ListBookCommandParser` parses this command, and returns an `ListBookCommand`. Exception will be thrown if the command is invalid.

Step 4. The `ListBookCommand` executes. The command checks all the book in the `BookShelf`, filters out books whose name contain `Life` based on `BookNameContainsKeywordPredicate`.

==== Design Considerations

===== Aspect: How to select books.

* **Alternative 1 (current choice):** Only books match all criteria provided will be selected.
** Pros: Easy to test and manage.
** Cons: Such searching technique may fail to provide the user with enough inforamtion.
* **Alternative 2:** Select books that match at least one criteria.
** Pros: More likely to find the book that the user is looking for.
** Cons: Hard to test and manage.
...
The summary feature is a command works to summarize the books in the bookshlef. The user can have a rough overview of what he/she has read.

Given below is an example usage and how summary feature works.
...
Step 4. The `SummaryCommandParser`returns an `SummaryCommand`. Exception will be thrown if the command is invalid.

Step 5. The `SummaryCommand` executes. The command will traverse all books in the book shelf, checks if there is authors or tags appear more than once and give relative details. The command will also show the highest rating of book in the book shelf.
...
** Pros: This prevent the command from accessing data managed by the model manager, thus independency of different parts is guaranteed.
** Cons: More methods are introduced into the model manager.
...
** Cons: This approach is unsafe and may leads to more bugs as it violates the principle of independency.
...
Step 4. The `AddReviewCommandParser`returns an `AddReviewCommand`. Exception will be thrown if the command is invalid.
...
* **Alternative 1 (current choice):** Have another independent list for reviews in the book shelf.
...
The ListReview Feature is currently combined with the previous SelectPerson.

Given below is an example usage and program behaviour:
...
.. Prerequisites: List all book using the `listBook` command. Multiple books in the list.
...
.. After the installation, `bookshelf.json` is not present. The file is initialized after main app runs for the first time. +
   At this point the file is loaded with the sample data.
...
.. If the developer modify the storage component, to make the data structure and content consistent with the newly updated code,+
   The developer should delete the file before relaunching the app.
.. If the file is corrupted, the developer or the user can manually fix the bookshelf.json to restore the data. +
   However, he or she should stick to the format specified or the data will not be read properly. +
   Therefore, change the `bookshelf.json` is not encouraged as such behavior will cause unexpected bugs.
