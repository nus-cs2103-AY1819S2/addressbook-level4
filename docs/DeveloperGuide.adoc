...
This state can be accessed when user executes the command `select DECK_NUMBER` and then `study`.

The following commands are supported under the study sessions:

* `close` - Finishes the study sessions and saves user score for each card under.
