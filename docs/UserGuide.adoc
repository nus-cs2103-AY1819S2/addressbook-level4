...
RestOrRant (ROR) is a desktop restaurant management application that allows *restaurant managers, waiters and cashiers* to perform *restaurant front-end operations* more efficiently. With ROR, you can easily manage table allocation, handle orders, track menu item availability and check restaurant statistics.
...
Welcome to RestOrRant! If you are just starting out, this quick start section will help you get RestOrRant up and running in no time at all. It will equip you with the basics so that you can quickly enjoy and master the features that ROR has to offer.
...
[NOTE]
You can only access Table Mode for tables that exist in the restaurant and are currently occupied. The tables can be updated in <<Restaurant Mode, Restaurant Mode>>.
...
The function of the order and table panels are as follows:

* *Order Panel*: This panel shows you the list of items ordered by all tables in RstOrRant, arranged in chronological order. This means that items that were ordered first appears at the top of the list. It also indicates the quantity left to serve for each item, and the background turns dark when the item is fully served.
* *Table  Panel*: This panel gives you an overview of all tables in ROR with their table numbers and respective occupancies.
...
As you set up ROR to reflect your physical restaurant or add tables in the event that there are new tables in your restaurant, this command allows you to add tables in ROR by specifying the number of seats at each table.
...
a| a| For example, ROR initially has 35 tables when you open it for the first time as shown in the image below.

image::AddTableExampleBefore.png[]
...
image::AddTableExampleResultDisplay.png[]
The Table Panel will also be updated and if you were to scroll down on the table panel, you will see tables 36 to 45 added as shown in  the image below.

image::AddTableExampleAfter.png[]

a| a| Alternatively, you can add also add just one table at a time. For example, you can use the command `addTable 4` to add one table with 4 seats.
|===

[[EditPax]]
==== Editing the occupancy : `editPax`

During the operation of your restaurant, this command helps you to update the occupancies of the tables in ROR when customers enter your restaurant.

[IMPORTANT]
Changing the table's occupancy to 0 will automatically clear all records of its orders which will not be included in ROR's statistics.
...
[NOTE]
The specified table has to exist in ROR and your specified number of customers cannot be greater than the number of seats available at the table.
...
a| a| For example, Table 1 in ROR is initially unoccupied as shown below where the table's status is 0/2.

image::EditPaxExampleBefore.png[]
...
image::EditPaxExampleResultDisplay.png[]
...
image::EditPaxExampleAfter.png[]
|===

[[EditSeats]]
==== Editing the available seats : `editSeats`

In the event that there are changes to any table in your restaurant in terms of the number of seats at the table, this command can help you make sure ROR is updated to reflect your physical restaurant.
...
[NOTE]
The specified table has to exist in ROR and the specified number of seats has to be more than the number of customers already seated at the table.
...
a| a| For example, Table 2 in ROR is initially occupied as shown below where the table's status is 2/2.

image::EditSeatsExampleBefore.png[]
...
image::EditSeatsExampleResultDisplay.png[]
Table 2 in the table panel will then display the updated table information as shown below.

image::EditSeatsExampleAfter.png[]
|===

[[ClearTable]]
==== Clearing all the tables : `clearTable`

In the event that you have undergone massive changes in the restaurant, you can use this command to remove all tables in ROR.
...
a| a| For example, when all the customers have left the restaurant, ROR will have all unoccupied tables as shown below.

image::ClearTableExampleBefore.png[]

After you enter the command `clearTable`, a success message will be displayed and the table panel will become completely empty as shown below.

image::ClearTableExampleAfter.png[]
|===
...
[NOTE]
This command finds the smallest table able to accommodate the specified customers with the smallest table number.
...
a| a| For example, in the sample layout we provided, the next available table that can fit 3 customers is table 25 since it is the smallest unoccupied table that can accommodate 3 customers with the smallest table number as shown below. 

image::SpaceForExampleBefore.png[]
...
image::SpaceForExampleResultDisplay.png[]
In the table panel, you will see that table 25 will have been updated to the correct table's occupancy and change to a slightly darker colour to indicate that it is occupied.

image::SpaceForExampleAfter.png[]
|===
// end::restaurantmode[]
