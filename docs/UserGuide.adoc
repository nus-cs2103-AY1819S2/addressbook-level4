...
* `YEAR` has to be be within 2000 and the current year.
* `MONTH` has to be within 1 to 12.
* `DAY` has to be within 0 to 31 (Special case: 29, 30 and 31 are only vaild if it exists in the month of a specified year).
****
...
[width="100%",cols="5%,95%", grid=none]
|===
ifdef::env-github[| :white_check_mark: a| icon:check[role="green", size="2x"] a| The user interface switches to <<Statistics Mode, Statistics Mode>>.]
...
new customers to order at the table. The money earned from the bill will automatically be added in the
RestOrRant's daily revenue statistics.
...
* *Revenue Panel*: This panel displays the revenue from the past 30 years, months and days for you to study the revenue trends.

[NOTE]
By default the Statistics Mode displays revenue from the past 30 days in the Revenue pPnel unless you execute
<<Yearly, `yearly`>>, <<Monthly, `monthly`>> or <<Daily, `daily`>> commands to change its display respectively.

Here are the commands that you can use in this mode to efficiently study the statistical data of your restaurant.
...
ifdef::env-github[| :white_check_mark: a|]
ifndef::env-github[a| icon:check[role="green", size="2x"] a|]
You will see a success message in the Results Display and the Revenue Panel changes to display the revenue earned from
 the last 30 years starting from the most recent record.

a| a| Notice that the first yearly revenue is the most recent year recorded. This saves you from the trouble of
...
ifndef::env-github[a| icon:check[role="green", size="2x"] a|]
You will see a success message in the Results Display and the Revenue Panel changes to display the revenue earned from
 the last 30 months starting from the most recent record.

a| a| Notice that the first monthly revenue is the most recent year recorded. This saves you from the trouble of
scrolling through the rest of the record.
...
ifndef::env-github[a| icon:check[role="green", size="2x"] a|]
You will see a success message in the Results Display and the Revenue Panel changes to display the revenue earned from
 the last 30 days starting from the most recent record.

a| a| Notice that the first daily revenue is the most recent year recorded. This saves you from the trouble of
scrolling through the rest of the record.
...
[[bill]] Bill::
The amount of money that dining customers have to pay based on their orders
...
|=======================================================================
^|Command ^|Shortcut ^|Usage ^|Example
a|<<Revenue, `revenue`>> a|`*r*` a|`revenue [d/DAY] [m/MONTH] [y/YEAR]` a|`revenue d/1 m/1 y/2019` +
Returns the revenue earned on 01 January 2019
a|<<Yearly, `yearly`>> a|`*y*` a|`yearly` a|
Displays revenue from last 30 years in the Revenue Panel
a|<<Monthly, `monthly`>> a|`*m*` a|`monthly` a|
Displays revenue from last 30 months in the Revenue Panel
a|<<Daily, `daily`>> a|`*d*` a|`daily` a|
Displays revenue from last 30 days in the Revenue Panel
|=======================================================================
