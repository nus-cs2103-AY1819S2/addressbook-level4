= Arielyte Tsen Chung Ming - Project Portfolio
:site-section: AboutUs
:toc:
:toc-title: Project Portfolio
:toc-placement: preamble
:sectnums:
:imagesDir: ../images
:stylesDir: ../stylesheets
:sourceDir: ../src/main/java
:xrefstyle: full
:experimental:
ifdef::env-github[]
:tip-caption: :bulb:
:note-caption: :information_source:
:important-caption: :heavy_exclamation_mark:
:caution-caption: :fire:
:warning-caption: :warning:
endif::[]
:repoURL: https://github.com/CS2103-AY1819S2-W11-3/main
:source-highlighter: rouge
:doctype: article
:reproducible:

= PROJECT: TravelBuddy
---
== Overview

This document serves to highlight my contributions to *TravelBuddy*, a travel log application that lets users efficiently manage their travel records, i.e. the places that they have visited. Additional features built into TravelBuddy allows them to search through the records, attach photos and view statistical models. This project is part of the CS2103 Software Engineering module in NUS, in which link:https://cs2103-ay1819s2-w11-3.github.io/main/AboutUs.html[my team and I] were tasked to morph a given link:https://se-education.org/addressbook-level4/[address book] product into a new product -- TravelBuddy. The complete source code for TravelBuddy is available on link:{repoURL}[GitHub].

My role was to design the chart feature. The following sections describes these enhancements in more detail, as well as the relevant sections I have added to the user and developer guides in relation to these enhancements.

== Summary of Contributions

=== Major Enhancement
The major enhancement I did was adding the ability to generate charts in real-time and via the `generate` command, details of which can be found below:

*What it does:* It allows users to generate charts based on their travel data that they have accumulated in the product.

*Justification:* This feature allows users to gather insights into their travel patterns. It improves the user-friendliness of the product, gives the product additional functionality and is the differentiating factor among other travel log products on the market.

*Highlights:* Due to the real-time update of the charts, this enhancement affects existing and future commands. Furthermore, the complexity of this enhancement is due to the fact that it links up with all the components of the existing architecture, as described below:

* *Logic:* The chart feature can be activated via the `generate` command. Hence, a parser is required to parse the command.
* *Model:* Any changes made to the travel records has to be reflected in the charts. Hence, relevant data is extracted from the latest version of the travel records.
* *Storage:* The extracted data has to be stored into the charts' respective JSON files.
* *User Interface (UI):* The charts have to be displayed to the users.

The enhancement required an in-depth analysis of various design alternatives. The challenge was to redesign the architecture to suit the enhancement while still maintaining the integrity of other existing commands and features.

*Credits:* link:https://github.com/google/gson[Gson], a third-party library, was used in the enhancement of this product to convert Java Objects into JSON and back.

*Code contributed to major enhancement*: [link:https://nus-cs2103-ay1819s2.github.io/cs2103-dashboard/#search=chung-ming&sort=displayName&since=2019-02-10&until=2019-04-10&timeframe=day&reverse=false&repoSort=true[RepoSense]] [link:{repoURL}/pulls?utf8=%E2%9C%93&q=is%3Apr+author%3Achung-ming+-label%3Atype.MinorEnhancement+-label%3Atype.Testing[Functional code]] [link:{repoURL}/pulls?utf8=%E2%9C%93&q=is%3Apr+author%3Achung-ming+label%3Atype.Testing[Test code]]

=== Minor Enhancement

The minor enhancement I did was converting the `add` command's Email parameter in the previous address book product to a Description parameter in the current product.

*Code contributed to minor enhancement*: [link:{repoURL}/pulls?utf8=%E2%9C%93&q=is%3Apr+author%3Achung-ming+label%3Atype.MinorEnhancement[Functional code]]

*Other contributions*: The details of other contributions can be found in <<OtherContributions>> below:

[[OtherContributions]]
[width="100%", cols="20%,<80%", reftext="Table 2.2.1", caption="Table 2.2.1: "]
.Details of Other Contributions
|=======================================================================

|*Project management*
|Managed releases on GitHub (Releases: link:{repoURL}/releases/tag/v1.2[`v1.2`], link:{repoURL}/releases/tag/v1.2.1[`v1.2.1`], link:{repoURL}/releases/tag/v1.3.1[`v1.3.1`] and link:{repoURL}/releases/tag/1.4[`v1.4`])

|*Enhancements to existing features*
a|* Updated the GUI color scheme to a `LightTheme.css` (Pull request: link:{repoURL}/pull/88[#88]) +
* Added command aliases to both new and existing commands (Pull requests: link:{repoURL}/pull/5[#5] and link:{repoURL}/pull/163[#163])

|*Documentation*
a|* Improved readability to existing contents of the User Guide (Pull request: link:{repoURL}/pull/165[#165]) +
* Added user stories and use cases to the Developer Guide (Pull request: link:{repoURL}/pull/170[#170])

|*Community*
a|* Reviewed PRs (with non-trivial review comments) (Pull request: link:{repoURL}/pull/115#discussion_r271991549[#115]) +
* Contributed to forum discussions (Example: link:https://github.com/nus-cs2103-AY1819S2/forum/issues/37#issuecomment-474714683[1]) +
* Reported bugs and suggestions for other teams (Examples:
https://github.com/nus-cs2103-AY1819S2/pe-dry-run/issues/268[1],
https://github.com/nus-cs2103-AY1819S2/pe-dry-run/issues/318[2],
https://github.com/nus-cs2103-AY1819S2/pe-dry-run/issues/486[3],
https://github.com/nus-cs2103-AY1819S2/pe-dry-run/issues/558[4],
https://github.com/nus-cs2103-AY1819S2/pe-dry-run/issues/632[5],
https://github.com/nus-cs2103-AY1819S2/pe-dry-run/issues/796[6],
https://github.com/nus-cs2103-AY1819S2/pe-dry-run/issues/968[7] and
https://github.com/nus-cs2103-AY1819S2/pe-dry-run/issues/18[8])

|*Tools*
a|* Integrated a third-party library (Gson) to the project (Pull request: {repoURL}/pull/80[#80]) +
* Integrated a new GitHub plugin (Codacy) to the team repo (Pull request: {repoURL}/pull/113[#113])

|=======================================================================

<<<
== Contributions to the User Guide

****
_Given below is the start of extract of sections I contributed to the User Guide. They showcase my ability to write documentation targeting end-users._
****

//include::../UserGuide.adoc[tag=intro]
//include::../UserGuide.adoc[tag=features]
include::../UserGuide.adoc[tag=generate]
//include::../UserGuide.adoc[tag=cheatsheet]
//include::../UserGuide.adoc[tag=commandFormats]

****
_This is the end of extract of the User Guide._
****

== Contributions to the Developer Guide

****
_Given below is the start of extract of sections I contributed to the Developer Guide. They showcase my ability to write documentation targeting end-users._
****

//include::../DeveloperGuide.adoc[tag=implementation]
include::../DeveloperGuide.adoc[tag=chartFeature]
//include::../DeveloperGuide.adoc[tag=user-stories]
//include::../DeveloperGuide.adoc[tag=use-cases]

****
_This is the end of extract of the Developer Guide._
****
