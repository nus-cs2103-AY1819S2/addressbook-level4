...
The filter feature allow users to filter the overall list with any specified fields, and also can filter by multiple fields.

The `FilterCommand` is able to filter the overall list according to the user's preference at a time.
...
** Cons: It only filters the overall list.
...
==== Introduction
We have implemented a `SortCommand` that allow users to sort the current shown list with specific fields.
...
Output: The list is sorted in ascending order by the phone numbers of the client.



[TIP]
`SortCommand` only supports sorting by date and phone number.
