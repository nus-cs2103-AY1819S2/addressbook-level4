...
. PREVIEW: displays both question and answer in file order (the same order as the cards in the storage file) but you will not be tested.
. DIFFICULT: displays cards labelled as difficult with both question and answer together but you will not be tested.
. REVIEW: tests the words which are urgent for you to review based on the srs value, meaning the due date for you to review these words are before the current time.
. LEARN: displays new cards with both question and answer first then you will be tested on these words.
...
Starts a quiz containing the first card in the country-capitals file that is new for the user. Firstly the question and answer of this card will display and then the user will be tested on this card.
* `start n/country-capitals-asia c/20 m/PREVIEW` +
Starts a quiz containing 20 cards which are the first 20 cards in the country-capitals-asia file. Question and answer will display together for all of these 20 cards.
