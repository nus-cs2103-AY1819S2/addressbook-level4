...
|`* * *` |clean user |delete a person |remove entries that I no longer need

|`* * *` |efficient user  |find a person by name |locate details of persons without having to go through the entire list

|`* * *` |sloppy user |add a contact into the app without having to specify all the contact information (i.e. all fields are optional) |add a contact even if I do not have the complete contact information
...
|`* * *` |lazy user |search for contacts by tags |find all the contacts with the same tag(s), e.g. search for "Landed" should return all contacts with "Landed" tag

|`* * *` |organised user |categorise my contacts into different groups (i.e. buyers, sellers, lessors, lessees) |keep track of my customers better

|`* * *` |clean user who wants to keep contacts for later use |hide or archive contacts when I currently do not need them |retrieve them when I need them again much later
...
|`*` |detailed user |add a remark to a contact |record more miscellaneous details about a contact
...
|`*` |real estate agent |search online for current market trends and prices of properties similar to mine |so that I can check the competitiveness of my properties to make improvements on my properties and make adjustments to my prices
...
2. TheRealApp adds contact into the contact list.
+
Use case ends.
...
2. TheRealApp requests for confirmation.
3. User confirms the request.
4. TheRealApp clears the entire contact list.
+
Use case ends.
...
2. TheRealApp edits the contact in the contact list.
+
Use case ends.
