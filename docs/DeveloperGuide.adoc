...
image::person-attributes.png[width=100%]
...
image::memberadd_validation.png[width=70%]
...
image::MemberFilter_Logic.png[width=100%]

*Model Component*

image::MemberFilter_Model.png[width=100%]
...
image::updatedUIcomponent.png[width=100%]
...
image::MemberSortCommand_LogicComponentSequenceDiagram.png[width=100%]
...
==== Design Considerations
*Aspect:* List Management
To execute command, there are various ways. Below are some options:

* *Alternative 1 (current choice)*: Go through Model Component

** Pros : Reduce reliance on logic for heavy data management.
** Cons : Could be challenging to implement necessary methods at the model component.

* *Alternative 2*: Go through Logic Component

** Pros: Easy to implement by retrieving the `UniquePersonList` to perform data management.
** Cons: Defeats the purpose of having logic and model components. The model component will be underused, while the logic component will be overused.
...
`Activity` with `Movie Panel` as `ActivityName`, `Club Room` as `ActivityLocation` would not be displayed
