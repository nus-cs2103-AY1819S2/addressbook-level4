...
a| a| For example, when all the customers have left the restaurant, all the tables in ROR will be unoccupied as shown below.

image::ClearTables_Before.png[]
...
In the table panel, you will see that table 25 will have been updated to the correct table's occupancy and changed to a slightly darker colour to indicate that it is occupied.

image::SpaceFor_After.png[]
|===

[[Reserve]]
==== Making a reservation : `reserve` _(Coming in v2.0)_

Another key features that many restaurants have is the ability to make reservations. Hence, a reserve function would be a necessary next step to make management of tables and customers more efficient.
...
[NOTE]
ROR will check for other reservations and availabilities of tables before allowing the user to make a reservation.
