...
* Patient id(pid) and doctor id(did) must be existing id in patient and doctor list.
* Date or visit must be a valid date in calender and must be a past or today. Cannot be a day in the future.
* On the same day, only one medical history can be maintained between a doctor and a patient. If the patient see one doctor more than one times on the same day, can edit the write up of the medical history.
* However, a patient can have multiple medical histories with different doctors on the same day.
* Write up is doctor's description about the medical record. Write up can be edited by edit-med-hist.
...
* `add-med-hist pid/3 did/8 d/2018-07-09 w/Had a fever with sorethroat. Sleeps late.`

=== Listing all medical histories or medical histories with specified constraint(s): `list-med-hist`

Shows a list of all medical histories or a list of all medical histories of one specified patient. +
Format: `list-med-hist [pid/PATIENT_ID] [did/DOCTOR_ID] []`
