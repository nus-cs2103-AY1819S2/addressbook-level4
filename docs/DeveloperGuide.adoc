...
1. User is in the home directory and wants to enter a test session on the first folder by executing the command `test 1`.
    ...
    * First, the `logic` unit checks if this is a valid folder index. If it is not, an `invalidFolderIndexException` is
thrown.
    * Second, the `logic` unit also checks if the user is already in a test session using
    `Model#checkIfInsideTestSession()` where this `test` command would be rendered invalid.


2. The application then enters this folder by setting `ActiveCardFolderIndex` to this current index.
