...
Suppose as an administrative staff, you input the following commands as your first 2 requests for homecare into
HealthHub:
...
It is also worth noting that `statistics` is fully integrated with all request-related commands. This means that
regardless of how many commands you may decide to employ (such as `add`, `edit`, `delete`, `undo`, `redo`, `clr`),
`statistics` will automatically update itself and always provide real-time accurate statistical data of the health
conditions.
...
|_none_
|`assign`
|`hw/<index>` - `INDEX` of the health worker to be assigned +

`r/<index>` - `INDEX` of the request
