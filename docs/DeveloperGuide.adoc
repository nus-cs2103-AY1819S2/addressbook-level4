...
==== Introduction
We have implemented a `SortCommand` that allow users to sort the equipment list with specific keyword.
...
The `sort` command sorts the list by specified keyword in lexicographical order.
...
It then splits the arguments and checks if the next string is a valid keyword, else, it will throw a ParseException.
...
Step 5. The list is sorted by specified keyword (name) and returned to the GUI.

Test cases:

- Input: `sort`

Output: An error message will be shown to what keywords are available.
...
* **Alternative 1 (current choice): **Sorts by specific keyword by using the Comparator interface.
...
One of our main feature of *Equipment Manager* is the ability to have a visual representation on 1 or more equipments in a *Google Map*.
