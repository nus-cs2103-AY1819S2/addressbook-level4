...
Steps: +
1. When a user enters an add appointment command, the input is first validated by `AddAppointmentCommandParser`.
...
2. The Logic layer then executes the `AddAppointmentCommand`. Here, the appointment is checked against existing appointments in the model to ensure there are no duplicates.
The appointment object is then sent to the model layer.

3. The model adds the appointment to its internal list, and the UI layer is notified and updated through an `InvalidationListener`.

4. The Logic layer then stores the new appointment to disk using the Storage layer.
...
The next time the application is opened, the Storage layer will use `JsonAdaptedAppointment` again to convert appointments in the JSON file back in appointment objects.
...
When a user executes `add-doctor n/John Doe g/M p/98765432 y/3 s/'acupuncture`

Step1. LogicManager calls parseCommand("add-doctor") in docXParser.

Step2. Based on the COMMAND_WORD, docXParser builds a new AddDoctorCommandParser() and call function parse(arguments) of AddDoctorCommandParser;
...
list-doctor
When a user executes `list-doctor`

Step1. LogicManager calls parseCommand("list-doctor") in docXParser.

Step2. Based on the COMMAND_WORD, docXParser builds a new ListDoctorCommand() and calls execute() of ListDoctorCommand().

Step3. execute() calls Model Manager to update filtered doctor list to show all existing doctors in docX.
