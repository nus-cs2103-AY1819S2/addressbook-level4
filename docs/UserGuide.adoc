...
[NOTE]
An `undo` command after the test session will revert the whole test session so all scores for the cards will be
updated to it was before a test session. However, test session overall score shown in the report display remains the
same and is not affected by the undo command.
