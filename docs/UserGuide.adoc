...
Create a sub-folder named `data` inside the `braintrain` folder. Place the downloaded file lesson inside `data`. (i.e. `braintrain/data/`).
...
. Store the `userdata` folder into your transfer device(thumbdrive etc.).
. On another device, ensure that BrainTrain has been executed `at least once`.
. Transfer and overwrite the `userdata` folder.
