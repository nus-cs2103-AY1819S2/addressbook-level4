...
:mainWindowURL: {repoURL}/tree/master/src/main/java/seedu/pdf/ui/MainWindow.java
:helpWindowTestURL: {repoURL}/tree/master/src/test/java/seedu/pdf/ui/HelpWindowTest.java
...
.. Pdf will be loaded as *PDDocument*, which is an indicator if the application can perform other operations on the Pdf that need it to be handled as a `.pdf` file.
...
1. Identify the appropriate CommandParser that is requested by the user
  * In the case that the user requests the `find` command, the `PdfBookParser` will correctly parse
  the user-input to the `FindCommandParser`.
2. Call the `static` parse method within the appropriate CommandParser that is expected
to return the correct Command from the CommandParser.
  * Hence in the case of the `find` command. The `FindCommandParser` would return the
  `FindCommand` back to the `PdfBookParser`.

Upon receiving the `FindCommand` from the `PdfBookParser` the following steps are carried out.

1. The `execute` method, that is part of the abstract `Command` class, is invoked.
2. The parameters `Model` and `CommandHistory` are provided to the `execute` and the `FindCommand` executes its directive.
3. The `FindCommand` then updates the `Model` and `CommandHistory`.
4. Once the `Model` is updated, the user is able to view the visual changes of their desired actions within the application.
...
[NOTE]
An <<encrypted-file>> is a file that is protected with a password. The term `protected` and `encrypted` will be used interchangeably.
...
image::EncryptCommandSequenceDiagram.png[width="700"]

===== Consideration

There were considerations to allow for changing of password through the *EncryptCommand*. However, due to security reasons,
it was decided to focus on encrypting and unprotected Pdf. You will need to <<Decrypt feature>>
before encrypting it with a new password. This is to ensure your intent in changing the password,
as the current version *PDF++* does not provide `Forget Password` feature.

Furthermore, in ensuring a sophisticated protection system, several other features were introduced.
...
will not comprises your privacy has been found.
...
image::DecryptCommandSequenceDiagram.png[width="700"]

===== Consideration

There were considerations to allow for 2-Factor Authentication (2FA) for *DecryptCommand*.
...
your files. This ensures that only you/any authorised personnel can decrypt your files.

Furthermore, in ensuring a sophisticated protection system, several other features were introduced.

. <<History feature>> will not show the executed `DecryptCommand` which includes the password of the file.
. The <<Undo/Redo feature [Coming in v2.0]>> is temporarily disabled until a solution that
will not comprises your privacy has been found.
. Pressing up in the command box will not show the `DecryptCommand` that was previously executed.
