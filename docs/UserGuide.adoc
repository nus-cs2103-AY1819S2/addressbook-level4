...
* `find c/CS` +
Lists all CS modules in the module plan.

.Finding CS modules
image::find(c).png[width="800"]

* `find s/Y1S2` +
Lists all modules in Y1S2 in the module plan.
...
Recommends a list of modules that can be read based on the current module plan and course requirements.
Modules with unmet prerequisites or those already added (unless they have been failed) will not be recommended. +
Recommended modules are displayed in order of requirement type satisfied: Core, Breadth & Depth, Industry Experience, Faculty, General Education.
