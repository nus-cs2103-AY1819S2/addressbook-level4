...
The tag command allows you to add or remove a tag to a file that you may refer to by its index. +
Format (add tag) : `tag INDEX -a [t/ADD_TAG]...` +
Format (remove tag) : `tag INDEX -r [t/REMOVE_TAG]...`

****
* Tags must be a continuous word without spaces
****
...
The delete command lets you to delete a file from the application and your computer. The file that is to be deleted can be specified by its index on the list. +
Format: `delete INDEX`
