...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `PatientListPanel`, `MapPanel`, `HealthWorkerListPanel`, `RequestListPanel`, `LocationListPanel`, `StatusBarFooter`, etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
* can save the Address, Request Book data in json format and read it back.
...
The `add`, `edit` and `delete` command have been tweaked from the original AB4
 to include addition of `HealthWorker`, `Patient` and `Request`.
...
|`* * *` |user |Assign a health worker to an existing request|Close a currently open request by assigning a new health worker who has the skillset to fufill the request at the time of the request

|`* * *` |user |Register a new health worker|
|`* * *` |user |Register a new patient|
|`* * *` |user |Register a new caretaker|
...
|`* * *` |user |Register a new request from a caretaker|Key in a new request from a caretaker with the specific request and patient details
