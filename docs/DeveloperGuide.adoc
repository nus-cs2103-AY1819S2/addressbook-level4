...
session. The related methods are exposed in the `Model` interface.
...
* `#testCardFolder()` - begins a test session on the current card folder user is in and implicitly sorts the cards in
 it.
* `#endTestSession()` - ends the current test session.
** `currentTestedCardFolder` is set to null.
** `ModelManager#setCurrentTestedCard(null)` is invoked to set `currentTestedCard` to null.
** `inTestSession` is set to false.
** `ModelManager#setCardAsNotAnswered()` is invoked.
...
Model component, with `ModelManager#isInFolder()` and `Model#isInTestSession()` to check that user is inside a
folder and is not already in a test session. (This is not illustrated in the sequence diagram)
...
Step 7. `r` is returned to `LogicManager` which then terminates and returns `r` to the calling method.
    ...
    insert sequence diagram for logic and model interaction for test command

Step 8. The caller method here is `MainWindow`. Control is now transferred over to the UI component.

Step 9. As MainWindow sees `CommandResult` object `r` with the type `START_TEST_SESSION` and invokes
