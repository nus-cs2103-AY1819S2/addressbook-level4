...
public class VersionedRestOrRantTest {

    private final ReadOnlyRestOrRant addressBookWithAmy = new RestOrRantBuilder().withPerson(AMY).build();
    private final ReadOnlyRestOrRant addressBookWithBob = new RestOrRantBuilder().withPerson(BOB).build();
    private final ReadOnlyRestOrRant addressBookWithCarl = new RestOrRantBuilder().withPerson(CARL).build();
    private final ReadOnlyRestOrRant emptyAddressBook = new RestOrRantBuilder().build();

    @Test
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedRestOrRant versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedRestOrRant versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedRestOrRant.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedRestOrRant.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedRestOrRant versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedRestOrRant copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedRestOrRant differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedRestOrRant differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedRestOrRant versionedAddressBook,
                                             List<ReadOnlyRestOrRant> expectedStatesBeforePointer,
                                             ReadOnlyRestOrRant expectedCurrentState,
                                             List<ReadOnlyRestOrRant> expectedStatesAfterPointer) {
        // check state currently pointing at is correct
        assertEquals(new RestOrRant(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlyRestOrRant expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new RestOrRant(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlyRestOrRant expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new RestOrRant(versionedAddressBook));
        }
    ...
    /**
     * Creates and returns a {@code VersionedRestOrRant} with the {@code addressBookStates} added into it, and the
     * {@code VersionedRestOrRant#currentStatePointer} at the end of list.
     */
    private VersionedRestOrRant prepareAddressBookList(ReadOnlyRestOrRant... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedRestOrRant versionedAddressBook = new VersionedRestOrRant(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedRestOrRant versionedAddressBook, int count) {
