...
[source,java]
----
include::../src/main/java/seedu/address/model/player/Fleet.java[lines=204..209]
----
...
The Battle feature is split between several packages:
  ...
  * `seedu.address.battle.state`: the `BattleState` class is implemented here, and
...
image::StartGameSequenceDiagram.png[width=600]
...
image::HumanPerformAttackSequenceDiagram.png[width=300]
...
==== Design considerations
* **Current choice:** +
  `BattleManager` is stored under `Model`. Every time `Attack` or `Begin` commands
...
* **Alternative** +
  `BattleManager` is stored under `Logic`. Every time `Attack` or `Begin` commands
...
image::StatsCommandBox.PNG[width=200]
...
image::StatsWinScreen.PNG[width=350]
...
[none]
** 2a1. The game prevents the user from starting the battle, and
        informs them to place at least one ship.
