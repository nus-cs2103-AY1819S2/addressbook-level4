...
* |[NOTE]|Notes that provides additional information
* |[TIP]|Tips and tricks
* |[WARNING]|Warnings
* |[CODE]|Commands to input in the CLI
...
[[Design-Architecture]]
=== Architecture
The *_Architecture Diagram_* given below (Figure 1) explains the high-level design of ROR.
...
The UI component handles the User Interface of ROR.
...
The Logic component deals with the logic behind the execution of commands.
...
The Model component keeps a model of the current state of ROR.
...
* exposes an unmodifiable `ObservableList<Table>` that can be 'observed' e.g. the UI can be bound to this list so that the UI automatically updates when the data in the list change.
...
.. Test case: `delete 0` +
   Expected: No table is added. Error details shown in the status message. Status bar remains the same.
