...
It is the active state in TopDeck that determines which command is called.
...
For example, `CardsView` has a member `activeDeck`
which holds a reference to the deck opened in decks view. that commands such as `add` operate on.
...
`ModelManager` owns the sole instance of `ViewState` and is responsible for executing state transitions.
...
Strictly speaking, this method can be called from any state, not just decks view.

This abstraction barrier allows `ModelManager` to enforce mutual exclusion.
...
All methods are designed to work regardless of the state.
...
no runtime type checking is necessary.

As a consequence, transition methods can be called from any state.
State entry is handled by the constructors of each implementation of `ViewState`
...
The third ViewState is called `StudyState`.

This state can be accessed when user executes the command `study DECK_NUMBER`

The following commands are supported under the study sessions:

Study View supports the following operations:
* `done`
* [any other command]
* [difficulty rating]
...
The study mechanism is facilitated by `Deck`.

It extends `Model` with a  `cardShown` property, stored internally as an `ReadOnlyProperty` with generic type `Card`.
Additionally, it implements the following operations:

* `Deck#generateRandomCard()` -- Generates a random index based on the size of UniqueCardList in Deck
* `TopDeck#commit()` -- Saves the current TopDeck state in its history.
...
A `StudyDeck` command is returned after reading the command word, `study`.
Deck#generateRandomCard() is executed automatically as part of `StudyDeck` execution.

This modifies `cardShown` to a random card based on the index generated.
The `textShown` property in the UI is also modified right after.
The `textType` property in the UI is also modified to 0 for question.

Step 2. User executes the `[wildcard]` command to signal that he has come up with the answer.

Step 3. This command calls `Card#getAnswer()`
This changes the `textShown` property in the UI.
The `textType` property in the UI is also modified to 1 for answer.

Step 4. User enters the difficulty rating command.


==== Design considerations

===== Aspect: How study view is linked to the UI
...
`QuestionPanel` replaces the `ListPanel` when the active `ViewState` in the model is an instance
of `StudyView`.

`QuestionPanel` only has 1 property which is `textShown` and `textType`.
The first contains the words which will be shown, whether question or answer.

`textType` contains an enum to specify whether the panel is showing a question or an answer.
