...
- Add a new deck: `add DECK_NAME`
- Add a new card to a particular deck: `add q/QUESTION a/ANSWER`

The state in TopDeck determines which command is called.
The second command does not take the target deck as an argument as it is implicit in the state.

==== Current Implementation

States in TopDeck partition the functionalities that are exposed to a user.
The states are: decks view, cards view and study view.
Their responsibilites should be self-descriptive.
...
These states are implemented as subtypes of `ViewState`.
Each state holds transient data that is relevant only while the state is active.
For example, `CardsView` has `activeDeck` which is the deck that commands such as `add` operate on.

ModelManager owns the one instance of `ViewState` and exposes methods to handle state transitions.
...
* `TopDeck#commit()` -- Saves the current address book state in its history.
...
executes the `open 1` command to open the first deck(D1 in the diagram). This should change the
`ViewState` in the `ModelManager` from `DeckView` to `CardsView` and causes
`CardsView.activeDeck` to point to the first deck as per diagram 1. For more information, refer to
Deck's feature.

image::card_dg_diagrams/card_fig_1.png[width="800", caption="Figure 1"]
...
as per diagram 2 by calling `UniqueDeckList.setDeck(Deck target, Deck editedDeck)`.
...
image::card_dg_diagrams/card_fig_2.png[width="800", caption="Figure 2"]

Step 3. Next, the CardsView is updated creating a new `CardsView` that points to D3.

image::card_dg_diagrams/card_fig_3.png[width="800", caption="Figure 3"]
...
image::card_dg_diagrams/card_fig_4.png[width="800", caption="Figure 4"]
...
image::card_dg_diagrams/card_fig_5.png[width="800", caption="Figure 5"]
...
image::card_dg_diagrams/card_fig_6.png[width="800", caption="Figure 6"]
...
image::card_dg_diagrams/card_fig_7.png[width="800", caption="Figure 7"]

Step 9. Below is the final state of `ModelManager`:

image::card_dg_diagrams/card_fig_8.png[width="800", caption="Figure 8"]
