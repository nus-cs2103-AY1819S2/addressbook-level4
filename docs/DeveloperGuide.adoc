...
4. Autocomplete suggestions for setCategories
* Add new CategoriesAutoCompleteTextField class that extends TextField to implement autocomplete suggestions
* Add default suggestions to `Cuisine`, `Occasion`, `Price Range`.
...
Alternative 1 was chosen because it is better user experience to just type in a few characters instead of having to think
of what to type every time the user wants to set a price range. It also ensures that all restaurants' price ranges are visually
consistent.
