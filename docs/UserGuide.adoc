...
* Since the keyword matching search is done in a word-by-word manner, keywords should be words, not strings with white space in the middle. Though keywords with white space in between will not be rejected as invalid input, they will fail to return the searching result you are looking for. A following example will explain this constraint.
...
The book `Madame Bovary` and `Madame Butterfly` will be displayed.
* `listBook n/Madame Bovary` +
The book Madame Bovary will not be displayed. As the input `Madame Bovary` will be considered as a single word, neither `Madame` nor `Bovary` equals to the `Madame Bovary`.
