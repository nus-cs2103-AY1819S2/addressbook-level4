...
* Finally, upon succesfful execution, a `CommandResult` is returned to the `LogicManager` for other operations.
** If any errors occur mid-execution, a `CommandException` is thrown. These errors may occur due to:
*** Inavlid parameter inputs.
...
** An input command such as `select 1`.
* The `PdfBookParser` then decodes the instruction by identifying the keyword, which is
`select` and creates a new `SordCommandParser` to parse the necessary parameters.
* The `SordCommandParser` then parses both parameters and creates a `SordCommand` object that is
...
* Finally, upon succesful execution, a `CommandResult` is returned to the `LogicManager` for other operations.
** If any errors occur mid-execution, a `CommandException` is thrown. These errors may occur due to:
*** Inavlid parameter inputs such as invalid `CRITERIA` or `ORDER`.
