...
*Logic & Model Interaction:* :: Having discussed Logic and Model, we can now model the workflow of the `generate` command. This can be accomplished using an <<activity-diagram, activity diagram>>, as seen in <<GenerateCommandActivityDiagramFigure>> below.

[[GenerateCommandActivityDiagramFigure]]
[reftext="Figure 4.4.1"]
[caption="Figure 4.4.1: "]
.Activity Diagram for the `generate` command
image::GenerateCommandActivityDiagram.png[width="600"]
...
. The `GenerateCommand#execute()` method s invoked.
...
:: A code snippet for `saveCountryChart(filePath)` is shown below. A third-party library called Gson was used to convert Java Objects into JSON and back. The Gson API can be found https://www.javadoc.io/doc/com.google.code.gson/gson/2.8.5[here].
...
[width="100%",cols="10%,<50%,<50%",options="header"]
|=======================================================================
...
|*Pros*
|Ease-of-use. Users do not need to type an additional `generate` command after changes are made to the Place list. +

Accurate. The chart reflects the latest changes regardless of whether the user types the `generate` command.
|Computationally less intensive, as the charts are only generated when required.

|*Cons*
|Computationally more intensive, especially when the Place list is huge, as all three charts need to be regenerated every time a change is detected.
|Less user-friendly. Users will need to type the `generate` command for the charts to reflect the changes made to the Place list. +

Less accurate. The chart does not reflect the latest changes until the user types the `generate` command.

|*Example*
|<<generateCommand7>> shows the state of the bar charts before the command `edit 1 cc/USA` was executed, while <<generateCommand8>> shows the state of the bar chart, with USA included, after the `edit` command was executed.
|N.A.
|=======================================================================
