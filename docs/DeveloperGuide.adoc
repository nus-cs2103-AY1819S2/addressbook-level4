...
. The model adds the appointment to its internal list, and the UI layer is notified and updated through an `InvalidationListener`.

. The Logic layer then stores the new appointment to disk using the Storage layer.
