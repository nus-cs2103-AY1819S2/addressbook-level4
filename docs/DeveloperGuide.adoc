...
A map grid is a 2D array made up of `Cell` objects. This 2D array is encapsulated within the `MapGrid` class.
...
. Fix `invalidAndValidPersonAddressBook.json`, `typicalPersonsMapGrid.json`, `validAddressBook.json` etc., such that the JSON tests will not fail due to a missing `remark` field.
