...
class JsonSerializableAddressBook {
     ...
     */
    public JsonSerializableAddressBook(ReadOnlyBookShelf source) {
        persons.addAll(source.getPersonList().stream().map(JsonAdaptedPerson::new).collect(Collectors.toList()));
    }

    /**
     * Converts this address book into the model's {@code BookShelf} object.
     *
     * @throws IllegalValueException if there were any data constraints violated.
     */
    public BookShelf toModelType() throws IllegalValueException {
        BookShelf addressBook = new BookShelf();
