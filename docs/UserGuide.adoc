...
=== Showing summary of people : `summary`
...
Edits the description of an existing entry in people. +
...
Edits an existing entry in the people. +
...
Deletes an entry in the people. +
*Format*: `delete INDEX`
[NOTE]
====
...
The finance people and allocated budget will be saved in the hard disk automatically after any command that changes the data. There is no need to save manually

// tag::savingpatterns[]
=== Establish saving patterns based on people and derive projected savings/expenses in a time span `[coming in v2.0]`

The finance people for each month will be archived at the end of the month. This information will be used to derive the projected savings/expenses based on the typical spending pattern of the user.
// end::savingpatterns[]
...
The application will link up with secure methods of payment such as payLah, Visa/MasterCard, Amex or Paypal to allow transactions between users. By paying through this application, the transactions will be automatically updated on the people.
...
*A*: At the current version, the application is unable to support transactions people in different currency. User will need to calculate and input based on the local currency. This will be rectified in the subsequent version of our application.
