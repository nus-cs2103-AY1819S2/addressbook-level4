...
===== Aspect: Usage of a `currentTestedCard` on top of `currentTestedCardIndex`
...
** Cons: Not so reader friendly. Need to keep going to the list to get that index, which may take a long time if the list is long. Potentially lead to possible violation of the Law of Demeter where an object should only interact with objects that are closely related to it.
...
`MainWindow#handleStartTestSession()` to display the `currentTestedCard` question and hints to the user.
...
* **Alternative 1 (current choice)**: `test` is executed when inside a folder. The user doesn’t have to specify the
...
[NOTE]
If the user is not in a test session or already attempted an answer for the current card, this answer command would be
rendered invalid. Step 3 would not proceed and an error message is displayed.

Step 4. After both checks have passed, `ModelManager#setCardAsAnswered()` is invoked.

Step 5. ModelManager now marks the attempted answer by invoking `ModelManager#markAttemptedAnswer(attemptedAnswer)`.

Step 6. After knowing the result of the attempt, the `Logic` unit creates a new card exactly the same as the current
...
Step 7. To complete the update in the change in score, model#updateFilteredCard(PREDICATE_SHOW_ALL_CARDS) and
model#commitActiveCardFolder() are invoked.

    To elaborate further?

Step 6. `AnswerCommand` now creates a `CommandResult` object, `r` with either type `ANSWER_CORRECT` or `ANSWER_WRONG`
...
Step 9. If `MainWindow` sees that `CommandResult` object `r` has the type `ANSWER_CORRECT`, it invokes
