...
Step3: AddMedHistCommandParser parses the argument and get patient name, doctor name, time and a short write up.
AddMedHistCommandParser calls constructors of Patient, Doctor, Date and WriteUp and then calls the constructor of MedicalHistory.
Then, AddMedHistCommandParser calls AddMedHistCommand(medicalHistory).
