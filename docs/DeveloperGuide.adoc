...
`DisplayListPanel`: Displays the charts and bars that are called by `generate`

'ExtendedPlacePanel': Display an expanded view of all Place data. Photo attachments to be included here as well in future releases (beyond v1.3).

`BrowserPanel`: Opens on clicking the PlaceListCard, on the right of the PlaceListPanel. Deprecated.
