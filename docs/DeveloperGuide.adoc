...
image::ObservableList.png[width="200"]
...
==== Current implementation
The module recommendation feature generates a list of `RecModule` representing the modules which the user is recommended to take,
based on the user's passed modules and course requirements. Each `RecModule` contains a `ModuleInfoCode` and its
corresponding `CourseReqType` satisfied. The entire list is printed on the GUI upon entering the `rec` command.

Whenever the `rec` command is called, a `RecModuleManager` is created which generates a `RecModulePredicate` and
a `RecModuleComparator` given the user's `Course` and `GradTrak`. This ensures that the recommendation list is updated
according to any changes in `Course` and `GradTrak`.

The `RecModulePredicate` is applied to a `FilteredList` of `RecModule` stored in `ModelManager`, to remove modules which
...
Within each test by the `RecModulePredicate`, a `RecModule` is checked for the `CourseReqType`(s) it satisfies using its `ModuleInfoCode`.
The `CourseReqType` with highest priority, which has yet to be fully completed, is then selected.
This `CourseReqType` is set into the `RecModule` being tested.
Subsequently, within each comparison by the `RecModuleComparator`, the `CourseReqType` satisfied by the two `RecModule` are compared.
...
** Use link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter#handleAddressBookChangedEvent(AddressBookChangedEvent)`] to update the courseReqCredits of modulesTaken whenever there are new changes to the addressbook.
...
** For system tests, modify link:{repoURL}/src/test/java/systemtests/AddressBookSystemTest.java[`AddressBookSystemTest`] to also verify the new total courseReqCredits of modulesTaken status bar.
