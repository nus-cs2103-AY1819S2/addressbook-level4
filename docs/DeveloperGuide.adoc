...
4a. SaveCommand calls the existing saveAddressBook() if the requested file is a ".json" file. +
4b. SaveCommand calls the new saveAsPdf() if the requested file is a ".pdf" file.
...
0. AddressBookParser creates ExportCommandParser. +
1. The ExportCommandParser uses ParserUtil to parse the user input. +
2. If the input is valid, ParserUtil creates a ParsedInOut object and returns it to ExportCommandParser. +
3. ExportCommandParser creates ImportCommand initialized with the ParsedInOut object. +
4. ExportCommand checks if INDEX_RANGE is `all`. +
4a. ExportCommand calls SaveCommand if INDEX_RANGE is `all`. Refer to Save feature. +
4b. Otherwise, ExportCommand add contents from the current storage to the temporary storage based on the input INDEX_RANGE. +
5. ExportCommand checks for if the requested file is a ".json" file or ".pdf" file. It also checks if the file is Read-only. +
5a. ExportCommand calls the existing saveAddressBook() if the requested file is a ".json" file. +
5b. ExportCommand calls the new saveAsPdf() if the requested file is a ".pdf" file.
