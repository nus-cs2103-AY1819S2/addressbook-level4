...
=== Report Operations
After testing, you can track your scoring performance against previous attempts using our report feature.

==== Display a folder score report for user : `report`

Displays a full-screen folder score report for the user. The report comprises a graph showing the last 10 folder scores, the latest score change, and a list of cards
and their individual scores. A folder score is the percentage of cards (in a folder) answered correctly in a test session.
