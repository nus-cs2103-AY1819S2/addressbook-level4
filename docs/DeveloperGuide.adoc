...
=== File Management
Although TeethHub already has a built-in auto-load and auto-save, implementing file management would give the user more flexibility with managing data. PDF export is also implemented so that the user would have an easier time making sense of the data when offline.
...
* **Alternative 1a:** Open/Import/Save/Export a .json file by calling JsonUtil or FileUtil.
...
* **Alternative 1b:** Save/Export .pdf file by calling PdfUtil or FileUtil. +
...
Current implementation: The Import/Export featues accept index ranges that are larger than the actual index range of the content to be imported/exported. +
* **Alternative 2:** Don't allow indexes out of range for Import/Export. +
** Pros: Less overhead and faster runtime as there are less characters in the regex to match.
** Cons: User would need to know the total amount of 
...
** Cons: User would need to know the total amount of patients in the external file if importing. Otherwise, the user might resort to inputting a very large index range, which would slow down runtime and increase memory needed due to the amount of indexes to process.

Current implementation: The Export feature calls the Save feature when the `all` keyword is detected. +
* **Alternative 3a:** Don't parse "all" keyword for Export. +
** Pros: Less overhead and faster runtime as there are less classes to go through. +
** Cons: User would need to know the total amount of 
...
0. AddressBookParser creates OpenCommandParser. +
1. The OpenCommandParser uses ParserUtil to parse the user input. +
...
0. AddressBookParser creates SaveCommandParser. +
1. The SaveCommandParser uses ParserUtil to parse the user input. +
2. If the input is valid, ParserUtil creates a ParsedInOut object and returns it to SaveCommandParser. +
3. SaveCommandParser creates SaveCommand initialized with the ParsedInOut object. +
4. SaveCommand checks for if the requested file is a ".json" file or ".pdf" file. It also checks if the file is Read-only. +
...
0. AddressBookParser creates ImportCommandParser. +
1. The ImportCommandParser uses ParserUtil to parse the user input. +
2. If the input is valid, ParserUtil creates a ParsedInOut object and returns it to ImportCommandParser. +
3. ImportCommandParser creates ImportCommand initialized with the ParsedInOut object. +
4. ImportCommand checks for if the requested file is a ".json" file, if the file exists, if it is a file, or if it can be read. +
5. ImportCommand calls the existing readAddressBook() on a temporary storage.
6. ImportCommand adds contents from the temporary storage to the current storage based on the input INDEX_RANGE.
6a. ImportCommand adds all contents from the temporary storage to the current storage if INDEX_RANGE is `all`.

=== Suggestion feature
===== Design Considerations

Current implementation: When the user makes multiple typing mistakes, Help window does not show automatically. +
* **Alternative 2:** Show Help window if the user inputs invalid commands `N` times in a row.
** Pros: Provide help to the user if the user needs it. +
** Cons: Might be rude.
