...
The arguments entered by the user are stored in a list of keywords and passed into `NameContainsKeywordsPredicate`, where the list is converted into a stream and individually matched to each entry in TravelBuddy.
...
The `search` command will call `LogicManager#execute()`, which in turn uses `TravelBuddyParser#parseCommand()`. Since the argument is `search`, `SearchCommandParser#parse(arguments)` will be called to parse the arguments to be used in `SearchCommand`.
...
The arguments entered by the user are stored in a list of keywords and passed into `RatingContainsKeywordsPredicate`, where the list is converted into a stream and individually matched to each entry in TravelBuddy.
...
The `searchr` command will call `LogicManager#execute()`, which in turn uses `TravelBuddyParser#parseCommand()`. Since
...
where the list is converted into a stream and individually matched to each entry in TravelBuddy.
...
It extends `Command` with a predicate that specifies the conditions of the country of the entry to be chosen.
The command is executed when the arguments parsed from the user input matches the command word of `SearchCountryCommand`,
which is `searchc`.

The arguments entered by the user are stored in a list of keywords and passed into `CountryContainsKeywordsPredicate`,
where the list is converted into a stream and individually matched to each entry in TravelBuddy.
...
It extends `Command` with a predicate that specifies the conditions of the year of the date visited of the entry to be chosen.
The command is executed when the arguments parsed from the user input matches the command word of `SearchYearCommand`,
which is `searchyear`.

The arguments entered by the user are stored in a list of keywords and passed into `YearContainsKeywordsPredicate`,
where the list is converted into a stream and individually matched to each entry in TravelBuddy.
...
Step 2. The user types the full add command `add n/NUS Computing cc/SGP dv/02/01/2018 r/5 d/No description a/NUS
School of Computing, COM1, 13 Computing Drive, 117417 t/school` to add the place to TravelBuddy and presses enter.
...
* **Alternative 1 (current choice):** Use java.util.Locales.
** Pros: Easy to implement and contains all country codes specified in ISO-3166.
** Cons: Slightly slow in locating country code.
* **Alternative 2:** Create own data structure containing most commonly traveled countries in the world.
** Pros: Locating country code in data structure would be fast.
...
. Fix `invalidAndValidPlaceAddressBook.json`, `typicalPlacesTravelBuddy.json`, `validAddressBook.json` etc., such that
