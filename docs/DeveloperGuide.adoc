...
By: `Team w09-3`      Since: `Jan 2019`      Licence: `MIT`
...
* can save the RestOrRant data in json format and read it back.
...
These operations are exposed in the `Model` interface as `Model#updateRestOrRant()`, `Model#undoAddressBook()` and `Model#redoAddressBook()` respectively.
...
Step 2. The user executes `delete 5` command to delete the 5th person in the address book. The `delete` command calls `Model#updateRestOrRant()`, causing the modified state of the address book after the `delete 5` command executes to be saved in the `addressBookStateList`, and the `currentStatePointer` is shifted to the newly inserted address book state.

image::UndoRedoNewCommand1StateListDiagram.png[width="800"]

Step 3. The user executes `add n/David ...` to add a new person. The `add` command also calls `Model#updateRestOrRant()`, causing another modified address book state to be saved into the `addressBookStateList`.

image::UndoRedoNewCommand2StateListDiagram.png[width="800"]

[NOTE]
If a command fails its execution, it will not call `Model#updateRestOrRant()`, so the address book state will not be saved into the `addressBookStateList`.
...
Step 5. The user then decides to execute the command `list`. Commands that do not modify the address book, such as `list`, will usually not call `Model#updateRestOrRant()`, `Model#undoAddressBook()` or `Model#redoAddressBook()`. Thus, the `addressBookStateList` remains unchanged.

image::UndoRedoNewCommand3StateListDiagram.png[width="800"]

Step 6. The user executes `clear`, which calls `Model#updateRestOrRant()`. Since the `currentStatePointer` is not pointing at the end of the `addressBookStateList`, all address book states after the `currentStatePointer` will be purged. We designed it this way because it no longer makes sense to redo the `add n/David ...` command. This is the behavior that most modern desktop applications follow.
...
// tag::editseats
=== Edit number of Customers feature
==== Current Implementation

The edit number of customers mechanism is facilitated by `UniqueTableList`. It stores the all tables currently in the system and ensures that there are no duplicated tables. Additionally, it implements the following operations:
...
Given below is an example usage scenario and how the edit number of customers mechanism behaves at each step.
...
The following sequence diagram shows how the edit number of customers operation works:
...
// tag::tablemode[]
=== Changing to Table Mode

==== Modes

The modes of the application are defined using a `Mode` enum instead of declaring integer constants. This is to make the code more readable as the enum values explicitly state the name of the mode (eg. `TABLE_MODE`, `MENU_MODE`) and to easily identify incorrect modes since the set of valid modes is restricted to the enum values.

The current mode of the application is stored as a private attribute `Mode mode` in `LogicManager`. It is stored in the `Logic` component rather than the `Model` component because modes are used to organise the available commands into their respective features, and the check for which a command is valid in a particular mode is done in `Logic`. With this implementation, `Model` does not need to know what the current mode is at all and this reduces coupling.

==== Changing mode

When the mode of the application is changed, we need to update the:
...
When the user wants to perform order-related operations (eg. add an item to the table's order), the user will first have to switch to Table Mode by typing the `tableMode` command (or its shortcut command `TM`) followed by the table number of the table that the user wants to update.

[NOTE]
The `tableMode` command can be used from any mode, even in Table Mode of a different table number. If the application is already in Table Mode of the same table number, an error message will be shown to the user.
...
The `LogicManager` will receive the command from the UI and parse the command to obtain the `TableModeCommand` object to be executed. The `TableModeCommand` contains a `TableNumber` attribute that indicates the table number of the target table to switch to. In this case, it will represent table number 3.

The `TableModeCommand` is then executed. It first retrieves the `Table` that corresponds to its `TableNumber` attribute. This is done by going through `Model#getRestOrRant()` to obtain the `ReadOnlyRestOrRant`, whose `getTables()` method can be called to retrieve `ReadOnlyTables` that contains the list of all the tables in the restaurant. The `Table` can then be retrieved by passing `TableNumber` into `ReadOnlyTables#getTableFromNumber()`.
...
`TableModeCommand` then calls `Model#updateMode()` to trigger the listener so that `LogicManager` knows that there has been a change in mode. When that is done, it generates a `CommandResult` whose `toMode` attribute is set to `TABLE_MODE` and this is returned to the `LogicManager`. Since the listener has set `modeModified` to true, `LogicManager` calls `changeMode()` to update its `mode` attribute to `TABLE_MODE`.

Finally, the `CommandResult` is returned to `MainWindow` in the UI and since the `CommandResult` contains a non-null `toMode` attribute, it will call `handleChangeMode()` to update the panels on display.
...
. Add a shorthand equivalent alias for each of the individual commands. For example, besides typing `clearTable`, the user can also type `clear` to remove all tables when they are in Restaurant Mode.
+
****
* Hints
** Just like we store each individual command word constant `COMMAND_WORD` inside `*Command.java` (e.g.  link:{repoURL}/src/main/java/seedu/address/logic/commands/AddTableCommand.java[`AddTableCommand#COMMAND_WORD`], link:{repoURL}/src/main/java/seedu/address/logic/commands/AddToOrderCommand.java[`AddToOrderCommand#COMMAND_WORD`]), you need a new constant for aliases as well (e.g. `AddTableCommand#COMMAND_ALIAS`).
...
|`* * *` |waiter |view the status of the tableList |know which tableList are free/occupied and which tableList are waiting for orders/has received their orders

|`* * *` |waiter |change status of tableList |update the availability of tableList
...
|`*` |waiter |reserve tableList |see which tableList can be provided to other subsequent customers
|=======================================================================
...
[discrete]
=== Use case: Switching to Restaurant Mode

*MSS*

1. User enters command to change to Restaurant Mode.
2. RestOrRant programs changes mode and indicates the mode it has changed to.
+
User case ends.

*Extensions*
[none]
* 2a. The program is already in Restaurant Mode.
+
[none]
** 2a1. RestOrRant returns an error message that it is already in Restaurant Mode.
+
Use case ends.

[discrete]
=== Use case: Switching to Menu Mode

*MSS*

1. User enters the menuMode command to switch mode to menu mode
2. RestOrRant changes the UI to show the menu in the list panel, and allows addToMenu command.
+
Use case ends.

*Extensions*
[none]
* 1a. The program mode is already in Menu Mode.
+
[none]
** 1a1. RestOrRant returns an message that program is already in menu mode.
+
Use case ends.


[discrete]
=== Use case: Adding tables to RestOrRant

*MSS*

1. User enters command to add a table to the restaurant
2. RestOrRant returns success message that the table is added, providing the table number and table status.

[discrete]

[discrete]
=== Use case: Checking for available tables

*MSS*

1.  User enters command to list all tableList in the restaurant.
2.  RestOrRant returns the status (number of customers at the table/number of seats at the table) of all tableList in the restaurant.
+
Use case ends.
...
+
[none]
** 2a1. RestOrRant returns an error message that there are no tableList in the restaurant.
+
Use case ends.

[discrete]
=== Use case: Taking an order

*MSS*

1.  User enters the table number, along with item codes and quantities of the menu items.
2.  RestOrRant updates the specified table's order with the menu items and displays the new order list.
+
Use case ends.
...
[none]
* 1a. The item code(s) is/are invalid.
+
[none]
** 1a1. RestOrRant displays the item codes that are invalid and remove them from the list of item codes to add to the order.
+
Use case resumes from step 2.

[discrete]
=== Use case: Adding Items to Menu
...
[discrete]
=== Use case: Check Statistics

*MSS*

1.  User enters the statistic command with specific optional arguements (year, month and date).
2.  RestOrRant collates data from the stated period, calculates all the statistics and prints out the calculated data in a neat presentation for the user to refer to.
+
Use case ends.
...
[[order]] Order::
The list of menu items ordered by dining customers ata table
