...
workers registered in Health Hub, or `request`, `r`, `2` to display all
registered requests.
...
immediately reflected in the list shown on the Health Hub GUI.
...
Using the `setreminder` command, you can set a reminder alert to go off at a certain time
...
In the example below, a reminder has been set to go off at 1.30PM with
