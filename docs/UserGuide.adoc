...
[[Features]]
== Features
This section describes all the commands available in FomoFoto and examples on how to use them. Commands are listed in alphanumeric order.

====
*Command Format*

* Words in `UPPER_CASE` are the parameters to be supplied by the user. +
e.g. For `import FILEPATH`, `FILEPATH` is a parameter which can be used as `import desktop/imagealbum/image1`.
* Items in square brackets are optional. +
e.g `brightness [BRIGHTNESS_RATIO]` can be used as `brightness` or `brightness 0.9`.
...
Adjusts the brightness of the opened image based on a decimal value. If the brightness value is not specified, a preset decimal value of 1.1 will be used. Any decimal value that is less than 1 (e.g. 0.5) will reduce the brightness of the image while any decimal value that is more than 1 (e.g. 1.9) will increase the brightness of the image. +
...
Adjusts the contrast of the opened image based on a decimal value. If the contrast value is not specified, a preset decimal value of 1.1 will be used. Any decimal value that is lower than 1 (e.g. 0.5) will reduce the contrast of the image while any decimal value that is higher than 1 (e.g. 1.9) will increase the contrast of the image.+
...
_Figure 10_ shows an example of how FomoFoto does not allow saving with invalid names. Ensure that your file name is valid before saving. There should be a message to indicate that the save is successful as seen in _Figure 11_. To overwrite the original image, simply use `save` without the file name.

// end::UserSave[]

// tag::SavePreset[]
=== Save Preset: `savepreset`
Saves a list of commands that was used to edit the opened image under a specified name so that you can apply them on other images in the future. The specified name given must not be used before. +
