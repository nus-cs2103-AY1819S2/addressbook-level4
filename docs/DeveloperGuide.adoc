...
.Component interactions for `deleteBook n/Alice` command
...
Given below is the Sequence Diagram for interactions within the `Logic` component for the `execute("deleteBook n/Alice")` API call.

.Interactions Inside the Logic Component for the `deleteBook n/Alice` Command
