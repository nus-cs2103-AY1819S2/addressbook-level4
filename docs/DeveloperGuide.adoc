...
* <<Design-Model,*`CurrentEdit` and `Album` *>>: Holds the data of images in the App in memory.
...
The `Model` component consists of two classes:
*  The `CurrentEdit`class which holds the image that the App is currently editing in-memory.
*  The `Album` class which stores the pool of images that has been imported into the App.
