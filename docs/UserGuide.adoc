...
[IMPORTANT]
====
If more than one parameter are given by user input, e.g. (`set $/500 $/400`), the latest parameter is taken.
====
// end::set[]
...
You can also specify a report period by specifying the number of days or months. However, these parameters are optional.

You may also type `summary` in the command box to exit the summary view mode, as shown in the diagram below.
...
* Adding more entries while in the summary view mode will update the pie chart statistics automatically, if the expense falls within the specified report period
====
...
With this feature, multiple users will be able to use _In-Credit-Ble_ on the same computer without being able to access or alter each other's data.
...
| Show summary of records | `summary` _or_ `summary #/PERIOD_AMOUNT p/PERIOD` | `overview` | `summary #/5 p/d` +
