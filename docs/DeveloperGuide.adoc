...
The initialise method in `MapGrid` will then do a deep copy of the passed in 2D array to initialise the underlying 2D array of `Cell`.
...
`InitialiseMapCommand` will then use this method to get the underlying 2D array and populate it from outside of the `MapGrid` class.
