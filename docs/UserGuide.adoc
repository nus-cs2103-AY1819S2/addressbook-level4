...
*reminders and appointments* panel. Appointments are coloured blue, medicine alarms are coloured red
...
[NOTE]
Similar to `<<listapp, listapp>>`, the only valid keywords for `FORMAT` are `day`, `week`, or `month`.
...
The reminders on the sidebar are identified with an index, which you can use to specify the reminder to be deleted.
