...
image::ObservableList.png[width="800"]
...
=== Adding/Removing/Checking Courses and Course Requirement Feature

==== Current Implementation
Users are able to add and remove and the course requirements and also check if that they have satisfied course requirements
from the list of modules that they have completed.

Similar to the implementation of the display modules feature, when the application is
...
will loaded as `Course` and `CourseRequirement` Objects (`Course` contains multiple `CourseRequirement` objects) and stored in an `ObservableList` inside of `ModelManager`.

In the ModelManager, we also store an instance of `ActiveCourse` which extends from Course. Only
one `Course` can be active since it represents the Course the user is currently
undertaking when using the Graduation Tracker. However, we allow for different multiple Course object to
allow extension of different courses not in the default implementation.

The core of the feature is the `CourseRequirement` feature; we use Java 9's `Predicate` to check against a `List<Module>`
to see whether the user fulfilled the `CourseRequirement` as well as `Function<List<Module>, String>` classes to identify the parts of `CourseRequirement` not satisfied.
Currently, there are two subclasses that extends `CourseRequirement`; `ContainsAll` and `ContainsAtLeast`. Given a list of regular expression,
`ContainsAll` ensure that there is at least 1 module taken by the user has module code that matches regex the list whereas on
the other hand `ContainsAtLeast` checks that at a minimum number of modules have
module code that fits the list. For now, users can only create `CourseRequirement` of the form
`ContainsAll`, `ContainsAtLeast` as well as combining them by using `or` and `and` functions defined in `CourseRequirement`.


==== Design Considerations
===== Aspect: Type of CourseRequirement Users can add/define
* **Alternative 1 (current choice): User can only create and combine limited types of requirements
...
** Con: Unless sufficiently many types that can be combined,
there might be requirements that are difficult to set properly.
* ** Alternative 2: Allowing user to check on any attributes of the Module that they have taken**
** Pros: Increased flexibility allows for more powerful expressions and increased usability for users side
** Cons: More code required; one more class for one additional attribute user can check

===== Aspect: ContainsAll/ContainsAtLeast isFulfilled() implementation
* **Alternative 1 (current choice): Check for regex matching of strings**
** Pros: Easier to implement in the storage stage since each regex string defines the predicate
** Cons: User might find it hard to isolate the correct regex to achieve the desired expression

* **Alternative 2: Stores module and check for set-containment against a set of ModuleInfo**
** Pros: Type-safe and can use pre-defined equality in 'ModuleInfo' class
** Cons: Modules can change frequently and be outdated quite quickly; might cause problem for
older CourseRequirement defined by user. Also alot of dependency issues in Storage and other areas
//end::CourseRequirement[]

//tag::recommend[]
=== Module recommendation feature
==== Current implementation
The module recommendation feature generates a list of module codes representing the modules which the user is recommended to take,
based on the user's module plan and course requirements. The entire list is printed on the GUI upon
entering the `rec` command.

Whenever the `rec` command is called, a `RecModulePredicate` and a `RecModuleComparator` are generated from the
`ModelManager` given the user's `Course` and `ModulePlan`. This ensures that the recommendation list is updated
according to any changes in `Course` and `ModulePlan`.

The `RecModulePredicate` is applied to a `FilteredList` of `ModuleInfo` stored in `ModelManager`, to remove modules which
the user is ineligible to take or does not contribute to the `CourseRequirement`.
This `FilteredList` is wrapped in a `SortedList` which accepts the `RecModuleComparator`, sorting the list in
decreasing priority of the `CourseReqType` satisfied by the module.
If a module satisfies more than one `CourseReqType`, the one with the highest priority is selected.
The order of priority is determined by the enum ordering in `CourseReqType`: Core, TE (covers TE/PE/FA), GE, UE.
Modules with equal priority are sorted lexicographically, considering the fact that lower-level modules should usually be taken first.

Within each test by the `RecModulePredicate`, a module is checked for the `CourseReqType`(s) it satisfies.
The `CourseReqType` with highest priority, which has yet to be fully completed, is then selected.
A mapping of the module code to this requirement is put into a `HashMap` stored in `Course`.
Subsequently, within each comparison by the `RecModuleComparator`, the `CourseReqType` satisfied by the two modules are
retrieved from the `HashMap` and compared. A new `HashMap` is created at the beginning of each `rec` command.
...
* Solution
** Modify the constructor of link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter`] to take in the courseReqCredits of persons when the application just started.
** Use link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter#handleAddressBookChangedEvent(AddressBookChangedEvent)`] to update the courseReqCredits of persons whenever there are new changes to the addressbook.
...
** For system tests, modify link:{repoURL}/src/test/java/systemtests/AddressBookSystemTest.java[`AddressBookSystemTest`] to also verify the new total courseReqCredits of persons status bar.
