...
previous test sessions. It makes use of Java FXML features to display score in a user-friendly manner. The following
 ...
 * `ReportDisplay#displayGraph()` -- Takes the last 10 or all test scores from `CardFolder`, whichever is lower, and
...
*Evaluation*: While in the long-run, the first alternative appears to better decision, time limitations make the Alternative 2 good enough for most practical use cases. The saved time was used to implement other features.
