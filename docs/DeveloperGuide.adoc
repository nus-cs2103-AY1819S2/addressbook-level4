...
change certain attributes tied to the PDF such as the `Name` and `Tag`s tied to a particular Pdf.

The Edit feature has the following syntax:
`edit <Index> n/<NewName> t/<NewTag1> t/<NewTag2>`
...
* `<NewTag*>` refers to the new `Tag`s you wish to add to 
...
Step 1: The user launches an application with either an existing set of `Pdf`s or a
new sample set of `Pdf`s stored within as shown below.

image::EditFeatureDefault.png[width="600"]
...
Step 3: Upon hitting enter to execute the command, the `EditCommandParser` splits
...
Step 5: If the validy check and the individual property check passes, the file is then
...
|`*` |user with many pdfs in the pdf book |sort pdfs by name |locate a pdf easily
|=======================================================================
