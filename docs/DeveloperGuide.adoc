...
[TIP]
The `.pptx` or `.xml` files used to create diagrams in this document can be found in the link:{repoURL}/docs/diagrams/[diagrams] folder. To update a diagram, modify the diagram in the pptx or xml file, select the objects of the diagram, and choose `Save as picture`.
...
1. A main issue with the current implementation of the `FindCommand` is it's slow speeds with respect to
...
2. Yet another enhancement to this feature you might wish to implement is the use of Optical Image Recognition,
