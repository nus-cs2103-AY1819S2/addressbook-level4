...
The 'listUnvisited' feature accepts the user current postal code and displays unreviewed restaurants nearest to the user postal code.
This mechanism is facilitated by `PostalDataSet`.
It contains a HashMap of `PostalData` which contains the x and y-coordinate of every postal code in Singapore.
...
Step 1. The user launches the application for the first time. The `PostalDataSet` will be initialised by with data from `data.json` through the `Storage` interface.
...
* Food lovers who want to record their personal food reviews and at the same time obtain personalised food recommendations based on their food review
...
4. FoodDiary retrieves data from Google, asks User whether details retrieved are correct.
5. FoodLover confirms
6. FoodDiary adds restaurant
+
Use case ends.
...
2. FoodDiary opens up website on one side of the UI
+
Use case ends
...
.  Food Lovers with fast typing speed should be able to accomplish tasks with commands faster than with a mouse or GUI.
