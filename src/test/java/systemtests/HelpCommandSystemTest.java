 ...
 */
public class HelpCommandSystemTest extends MediTabsSystemTest {
    ...
    public void openHelpWindow() {
        ...
        executeCommand(SelectCommand.COMMAND_WORD + " " + INDEX_FIRST_MEDICINE.getOneBased());
        assertEquals("", getCommandBox().getInput());
        assertCommandBoxShowsDefaultStyle();
        assertNotEquals(HelpCommand.SHOWING_HELP_MESSAGE, getResultDisplay().getText());
        assertTrue(getInformationPanel().isBatchTableLoaded());
        assertListMatching(getMedicineListPanel(), getModel().getFilteredMedicineList());

        // assert that the status bar too is updated correctly while the help window is open
        // note: the select command tested above does not update the status bar
        executeCommand(DeleteCommand.COMMAND_WORD + " " + INDEX_FIRST_MEDICINE.getOneBased());
