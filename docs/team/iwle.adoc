...
Hello there! My name is Ian and I am one of the developers of the Battleship game. The following document is a Project Portfolio Page of my contributions to Battleship.

== Project Overview

Battleship is the product of a semester-long project for the CS2103T (Software Engineering) module at the National University of Singapore. My team was tasked with adapting an existing phone addressbook application into an application of our choice. Our game, Battleship, is a revival of the classic retro game Battleships, where players try to destroy each other's ships by guessing the positions of their opponent's battleships.
...
** Highlights: The other features rely on the functionality provided by the Ship Management feature, such as creating an enemy player and processing the statistics for the game. The feature requires detailed planning as these features are built on top of the functionality provided by it.
...
include::../DeveloperGuide.adoc[tag=putShip]
include::../DeveloperGuide.adoc[tag=listTags]
_The sequence diagram for `listTags` has been omitted due to space constraints and because it is similar to the sequence diagram for `list`._
