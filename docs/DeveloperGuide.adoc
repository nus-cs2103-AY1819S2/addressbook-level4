...
// tag::import/exportlesson
=== Lesson CSV Storage Feature
The lesson storage feature manages the saving of lessons to disk for the application. This lets the user keep lessons between usage sessions in CSV files.

The user is then able to easily edit their lesson data in an external application, such as Excel.
...
The lessons are currently automatically loaded on start-up, and saved whenever lesson data is updated by the user and on exit.
