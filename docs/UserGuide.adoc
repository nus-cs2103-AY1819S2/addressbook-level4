...
* `addapp r/S9123456A d/2019-07-23 s/16:00 e/17:00 c/Weekly checkup` +

This adds an appointment allocated to the patient with NRIC S9123456A, on 23rd July 2019, from 4pm to 5pm. You can
...
The addition of the appointment could have failed as there is a conflict in timing with another existing appointment, or
the appointment timing is not within office hours which is from 9am to 6pm.
...
You should use the <<fa>> command to list all free appointment slots for your chosen dates to pick an appropriate
...
you want to see either by a format specifier and a date, or by an existing patient's NRIC.
...
Lists all appointments scheduled on the given week of 23rd July 2019, which is from 15th to 21st July. A week starts on a
...
You can filter the free slots you want to see by a format specifier and a date.
...
Lists all free appointment slots on the given week of 23rd July 2019, which is from 15th to 21st July. A week starts on a
Monday and ends on a Sunday.
{sp} +
...
If you want to see reminders not displayed on the sidebar, you can view them using the `listrem` command.
The reminders will be ordered and listed, with the earliest reminder at the top and the latest at the bottom.
You can filter the reminders you want to see by a format specifier and a date.
...
[NOTE]
The index is retrieved from the last `listrem` command. See: <<listrem>>.
