...
** Cons: All existing classes and methods which current work with `Person` needs to be re-written to work with the new `Patient` object.
Attributes and methods cannot be reused, and must be re-implemented.
Lastly, polymorphism cannot be applied if there is a need to deal with both persons and patients.
...
It can also save new records in order of when they are added, from most recent to oldest, simply using `List.add(0, Record)`.
** Cons: If the list get long over time, it might cause additional waiting time for our users when search for a specific record.
 ...
 ** Cons: It can be challenging for collaborators to understand, which is crucial for collaborative programming.
 It also creates an extra layer of complexity in order to display records in the order they were stored in the application by our users.
// end::patient[]
