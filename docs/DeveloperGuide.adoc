...
* <<Design-CurrentEdit,*`CurrentEdit`*>>: Holds the image that the App is currently editing in-memory.
* <<Design-Storage,*`Storage`*>>: Reads data from, and writes data to, the hard disk.
* <<Design-Album,*`Album`*>>: Stores the pool of images that has been imported into the App.
...
The _Sequence Diagram_ below shows how the components interact with each other for the scenarios where the user issues the commands `import C:\Users\XXX\Desktop\Pictures` and `rotate 90`.
...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `PersonListPanel`, `StatusBarFooter`, `ImagePanel`, `InitPanel`, `InformationPanel` and `ImageCard`.
...
* Utilizes the `Notifier` class which implements Java's `propertyChangeListener` to propagate changes.
* Components are registered as listeners as will update with modified data when triggered.
...
.  The command execution can affect the `CurrentEdit` (e.g. rotating an image).
...
*  The `assets` folder stores images that the user imported while the `temp` folder stores the image that the program is currently editing on.
...
The implementation of the import feature is largely similar to that of linux's `cp` command, with the exception the output path is fixed.

The command allows for users to pass in directory paths as well. All valid images in the directory will be copied over,
but the traversal is not recursive i.e. folders in that directory will not be searched.

It is facilitated by `Model`, `Album` and `ImportCommandParser` and contains the follow operation:

 * `Model#refreshAlbum()` - Fires the ~propertyChangeListener~ to update the information panel.
 * `ImportCommandParser#parse(String)` - Takes in an absolute path and copies the image to `assets` directory.
 * `Album#populateAlbum()` - Calls on the `Album` class to update the list of image based on photos in `assets` folder.
...
Example usage scenario:
 ...
 3. `Album` adds this image to its list of imported image.
 3. `Model` calls the `Notifier` class which fires a notifier to any listeners registered to it.
 4. `InformationPanel` updates to reflect the imported image.

image::ImportSequenceDiagram.png[width=800]

==== Design Considerations

* The command does not traverse inner directories.
** This design consideration was made to prevent excessive number of images being added to the program.
** This design consideration also mitigates cyclic graphs such as symbolic or hard links.

=== Tab feature
The Tab command allow users to cycle through tabs in `informationPanel` without using the mouse.

==== Current implementation
The implementation of the tab command is largely for convenience reasons and calls a single method.

It is facilitated by `Model` and contains the follow operation:

 * `Model#switchTab()` - Fires the `propertyChangeListener` to switch tabs in `informationPanel`.

Example usage scenario:

 1. User launches application and enters `tab`
 3. `Model` calls the `Notifier` class which fires a notifier to any listeners registered to it.
 4. `InformationPanel` updates to switch to the next available tab.

image::TabSequenceDiagram.png[width=800]

==== Design Considerations

* The command cycle tabs in a single fashion.
** This design consideration means that users cannot indicate which tab they would like to switch to.
** A potential feature could include a command argument i.e. `tab 2` which would open the second tab of `informationPanel`.
...
==== Current Implementation
The open function is facilitated by the ModelManager and contains the following operations:

* `Model#setCurrentImage(Image)` - Sets the `currentImage` to the opened image.

* `Model#createTempImage(Image)` - Creates a new temporary image inside the directory: "src/main/resources/temp/"

* `Model#displayImage()` - Displays the temporary image stored in directory: "src/main/resources/temp/"

Given below is an example usage scenario and how the open mechanism behaves at each step.

Step 1. The user executes `open sample1.png` and the open function checks whether the image exists in the directory: "src/main/resources/assets".

Step 2. The open function retrieves the image before it calls `Model#setCurrentImage(Image)` to store a new `Image` instance of the retrieved image into `currentImage`. Subsequent edits will be done on `currentImage`.

Step 3. `Model#createTempImage(Image)` creates a copy of `currentImage` as a temporary image file "temp_img" and stores it in directory: "src/main/resources/temp/".

Step 4. `Model#displayImage()` displays "temp_img" on Graphical User Interface.
...
Open command must be called before the user can edit the image. The command initialises `currentImage`, which all edits will be applied on. Users are unable to edit other images unless the open command is called again.
...
Refer to the example usage scenario and design considerations in <<ContrastCommand>>.
...
* `resizeCommand`: Allows user to resize images to the desired size while maintaining the proportions.

==== Rotate Command
This command allows the user to rotate images by specifying a degree (90, 180 or 270 only). Upon receiving an input degree by the user, the degree will be checked for its validity and will throw an error if the degree is not within the specified range. The command will then be added to the `Image`'s `List<EditCommands>` which saves the editing history of the targetted image.

Given below is an example usage scenario and how the command should behave at each step:


Step 1. When the users run the application and open an image to edit, a new `Image` object is created which stores the filename of the image,  history of edits `List<EditCommand>` and the image's metadata. This `Image` object will be set as the `currentImage` in the `Model` for editing.


Step 2. When the user enters the command (e.g. `rotate 90`), the entered command is parsed and the command will be executed.


Step 3. During execution, the `execute` method in the `rotateCommand` class calls for the methods `applyRotate` and `displayImage` in `Model`.


Step 4. The method `applyRotate` calls the external library https://github.com/rkalla/imgscalr[`ImgScalr`]'s class `rotate` which is applied on the image with the filepath `src/main/resources/temp/` + `currentImage#getFileName()`


Step 5. The method `displayImage` updates the user interface and the user interface will now display the image with the filepath `src/main/resources/temp/` + `currentImage.getFileName()`

==== Design Considerations

* Intermediate images have to be stored and edited in a temp folder first due to our `undo/redo` implementation.

* Furthermore, there was an attempt to self-implement the rotation feature, and thus allows the user to input a range of degree values from 0 to 360. However, there were issues with the saving images of odd sizes and we hence decided against it.

.Sequence Diagram for Rotate Command
image::RotateCommandSequenceDiagram.png[width="1280"]
...
image::UndoSeqTEMP.PNG[width="800"]
...
* **Alternative 2(current choice):** Save all transformation in a list and apply them when undo/redo is called.
** Pros: Will use less memory
** Cons: We must ensure that the implementation of each individual command are correct.

===== Aspect: Data structure to support the undo/redo commands

* **Alternative 1 (current choice):** Use a list to store the history of commands.
