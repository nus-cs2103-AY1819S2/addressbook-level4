...
|===
| _I also wrote the Table Mode section and the descriptions for the five Orders feature commands (including one command for v2.0). My introduction for the Table Mode section and descriptions of the `addToOrder` and `clearOrder` commands are replicated below while my full write-ups can be found https://github.com/CS2103-AY1819S2-W09-3/main/blob/master/docs/UserGuide.adoc#table-mode[here]._
|===

include::../UserGuide.adoc[tag=tablemode]

include::../UserGuide.adoc[tag=clearorder]
