...
* *Alternative 1 (current choice)*: User can only open and edit one image at a time.
...
** Cons: User is unable to open other images while editing.

* *Alternative 2* : User can open and edit multiple images.
** Pros: Users are able to edit multiple images simultaneously.
** Cons: Open has to distunguish whether user is opening an image from `assets` folder or `temp` folder. Requires many duplicates with different names. This may be confusing for the user.
...
image::ListFilesSequenceDiagram.png[width=800]
