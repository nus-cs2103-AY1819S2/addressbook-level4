...
[NOTE]
The term `protected` and `encrypted` has the same meaning and will be used
interchangeably.

The command `encrypt` allow you to protect your files with a password such that
they cannot be accessed without a password that you had specified.
Similarly, the command `decrypt` will let you remove the password that
you had previously set for the file.
...
Please refer to <<Encryption Guide>> for encryption guide and `Step-by-Step Guide --
