...
Format: `memberEdit INDEX n/NAME mn/MATRIC_NUMBER p/PHONE_NUMBER e/EMAIL yos/YEAR_OF_STUDY m/MAJOR [t/TAG]`
...
Finds members whose names contain any of the given keywords under the criteria. +
Format: `memberFind CRITERIA KEYWORD [MORE_KEYWORDS]`

****
* The search is case insensitive. e.g `hans` will match `Hans`
* The order of the keywords does not matter. e.g. `Hans Bo` will match `Bo Hans`
* Only the name and/or matric number is searched. CRITERIA is only `name` or `matricnum, case insensitive
* Partial words will be matched e.g. `Han` will match `Hans`
* members matching at least one keyword will be returned (i.e. `OR` search). e.g. `memberFind name Hans Bo` will return `Hans Gruber`, `Bo Yang` e.g. `memberFind matricnum A01` will return all members with matriculation number starting with `A01`
* 
****
...
* `memberFind matricnum A0123456Y` +
Returns `James Ong`
* `memberFind name John matric A0123456Y` +
Returns `John Doe`, `john` and `James Ong`
...
* *Find a member* : `memberFind CRITERIA KEYWORD [MORE_KEYWORDS]` +
e.g. `memberFind name John` or `memberFind matricnum A0123456Y`
