...
First, let's get you started with how to install and use HealthHub!
...
* `add healthworker` command: Adds a health worker to the application.
* `add request` command: Registers a new request for the hospital management to attend to.
* `assign request` command: Assigns an open request to an available health worker.

== v1.3 Release Notes

Added shorthand parameters for the different commands. For example,

* `add healthworker/h/1` command: Adds a health worker to the application.
* `add request/r/2` command: Registers a new request for the hospital management to attend to.
* `filter healthworker/h/1` command: Filters a healthworker by the individual's name, phone, organisation or
specialisation.

Other modified or extension commands include:

* `setreminder` command: Sets a reminder with a customised message for the hospital admin.
...
=== Setting a reminder for yourself: `setreminder`
If you wish to contact a patient on the day of his appointment to remind him that a health worker will be visting his
home, you can set a reminder on the application, with a customised message that you can include.

The app will set a reminder to alert you when the time has come.

Format: `setreminder t/<hh:mm:ss> m/<customised_message>` +

In the scenario below, a reminder has been set to go off at 6:00AM with the customised message to
"to call patient John Doe for his appointment".
