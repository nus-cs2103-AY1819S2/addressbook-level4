...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + NAME_DESC_BEDOK + RATING_DESC_BEDOK
                + DESCRIPTION_BEDOK + ADDRESS_DESC_BEDOK + TAG_DESC_MRT, new AddCommand(expectedPlace));

        // multiple names - last name accepted
        assertParseSuccess(parser, NAME_DESC_AMK + NAME_DESC_BEDOK + RATING_DESC_BEDOK + DESCRIPTION_BEDOK
                + ADDRESS_DESC_BEDOK + TAG_DESC_MRT, new AddCommand(expectedPlace));

        // multiple phones - last rating accepted
        assertParseSuccess(parser, NAME_DESC_BEDOK + RATING_DESC_AMK + RATING_DESC_BEDOK + DESCRIPTION_BEDOK
                + ADDRESS_DESC_BEDOK + TAG_DESC_MRT, new AddCommand(expectedPlace));

        // multiple descriptions - last description accepted
        assertParseSuccess(parser, NAME_DESC_BEDOK + RATING_DESC_BEDOK + DESCRIPTION_AMK + DESCRIPTION_BEDOK
                + ADDRESS_DESC_BEDOK + TAG_DESC_MRT, new AddCommand(expectedPlace));

        // multiple addresses - last address accepted
        assertParseSuccess(parser, NAME_DESC_BEDOK + RATING_DESC_BEDOK + DESCRIPTION_BEDOK + ADDRESS_DESC_AMK
                + ADDRESS_DESC_BEDOK + TAG_DESC_MRT, new AddCommand(expectedPlace));

        // multiple tags - all accepted
        Place expectedPlaceMultipleTags = new PlaceBuilder(BEDOK).withTags(VALID_TAG_MRT, VALID_TAG_EWL)
                .build();
        assertParseSuccess(parser, NAME_DESC_BEDOK + RATING_DESC_BEDOK + DESCRIPTION_BEDOK + ADDRESS_DESC_BEDOK
    ...
    public void parse_optionalFieldsMissing_success() {
        ...
        assertParseSuccess(parser, NAME_DESC_AMK + RATING_DESC_AMK + DESCRIPTION_AMK + ADDRESS_DESC_AMK,
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        // missing name prefix
        assertParseFailure(parser, VALID_NAME_BEDOK + RATING_DESC_BEDOK + DESCRIPTION_BEDOK
                + ADDRESS_DESC_BEDOK, expectedMessage);

        // missing rating prefix
        assertParseFailure(parser, NAME_DESC_BEDOK + VALID_RATING_BEDOK + DESCRIPTION_BEDOK
                + ADDRESS_DESC_BEDOK, expectedMessage);

        // missing description prefix
        assertParseFailure(parser, NAME_DESC_BEDOK + RATING_DESC_BEDOK + VALID_DESCRIPTION_BEDOK
                + ADDRESS_DESC_BEDOK, expectedMessage);

        // missing address prefix
        assertParseFailure(parser, NAME_DESC_BEDOK + RATING_DESC_BEDOK + DESCRIPTION_BEDOK
                + VALID_ADDRESS_BEDOK, expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_NAME_BEDOK + VALID_RATING_BEDOK + VALID_DESCRIPTION_BEDOK
                + VALID_ADDRESS_BEDOK, expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        // invalid name
        assertParseFailure(parser, INVALID_NAME_DESC + RATING_DESC_BEDOK + DESCRIPTION_BEDOK
                + ADDRESS_DESC_BEDOK + TAG_DESC_EWL + TAG_DESC_MRT, Name.MESSAGE_CONSTRAINTS);

        // invalid rating
        assertParseFailure(parser, NAME_DESC_BEDOK + INVALID_RATING_DESC + DESCRIPTION_BEDOK
                + ADDRESS_DESC_BEDOK + TAG_DESC_EWL + TAG_DESC_MRT, Rating.MESSAGE_CONSTRAINTS);

        // invalid description
        assertParseFailure(parser, NAME_DESC_BEDOK + RATING_DESC_BEDOK + INVALID_DESCRIPTION
        ...
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + NAME_DESC_BEDOK + RATING_DESC_BEDOK
                + DESCRIPTION_BEDOK + ADDRESS_DESC_BEDOK + TAG_DESC_EWL + TAG_DESC_MRT,
