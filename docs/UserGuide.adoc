...
Welcome to the *Know-It-All* User Guide! This document will equip you with what you need in order to use `v1.4` of the application. While some familiarity with command line programs will come in handy, simply adhere to the command formats specified in this guide and the rest will be a breeze to use.  Head over to <<Quick Start>> to get started.
...
Level: An intrinsic score tied to each flashcard tracking the user’s performance on the
flashcard.
