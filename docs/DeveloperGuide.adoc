...
The _Sequence Diagram_ below shows how the components interact with each other for the scenario where the user issues the command `delete n/Alice`.

.Component interactions for `delete n/Alice` command
...
Given below is the Sequence Diagram for interactions within the `Logic` component for the `execute("delete n/Alice")` API call.
...
* `VersionedBookShelf#commit()` -- Saves the current address book state in its history.
* `VersionedBookShelf#undo()` -- Restores the previous address book state from its history.
* `VersionedBookShelf#redo()` -- Restores a previously undone address book state from its history.
...
Step 1. The user launches the application for the first time. The `VersionedBookShelf` will be initialized with the initial address book state, and the `currentStatePointer` pointing to that single address book state.

image::UndoRedoStartingStateListDiagram.png[width="800"]

Step 2. The user executes `delete n/Alice` command to delete the book called Alice in the book list. The `delete` command calls `Model#commitBookShelf()`, causing the modified state of the book list after the `delete n/Alice` command executes to be saved in the `BookShelfStateList`, and the `currentStatePointer` is shifted to the newly inserted book list state.

image::UndoRedoNewCommand1StateListDiagram.png[width="800"]

Step 3. The user executes `add n/David ...` to add a new person. The `add` command also calls `Model#commitBookShelf()`, causing another modified address book state to be saved into the `BookShelfStateList`.

image::UndoRedoNewCommand2StateListDiagram.png[width="800"]

[NOTE]
If a command fails its execution, it will not call `Model#commitBookShelf()`, so the address book state will not be saved into the `BookShelfStateList`.
...
[NOTE]
If the `currentStatePointer` is at index 0, pointing to the initial address book state, then there are no previous address book states to restore. The `undo` command uses `Model#canundoBookShelf()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the undo.

The following sequence diagram shows how the undo operation works:

image::UndoRedoSequenceDiagram.png[width="800"]

The `redo` command does the opposite -- it calls `Model#redoBookShelf()`, which shifts the `currentStatePointer` once to the right, pointing to the previously undone state, and restores the address book to that state.

[NOTE]
If the `currentStatePointer` is at index `BookShelfStateList.size() - 1`, pointing to the latest address book state, then there are no undone address book states to restore. The `redo` command uses `Model#canredoBookShelf()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the redo.

Step 5. The user then decides to execute the command `list`. Commands that do not modify the address book, such as `list`, will usually not call `Model#commitBookShelf()`, `Model#undoBookShelf()` or `Model#redoBookShelf()`. Thus, the `BookShelfStateList` remains unchanged.

image::UndoRedoNewCommand3StateListDiagram.png[width="800"]

Step 6. The user executes `clear`, which calls `Model#commitBookShelf()`. Since the `currentStatePointer` is not pointing at the end of the `BookShelfStateList`, all address book states after the `currentStatePointer` will be purged. We designed it this way because it no longer makes sense to redo the `add n/David ...` command. This is the behavior that most modern desktop applications follow.
...
It contains `UniqueBookList` in which all the book data are stored and duplicates are not allowed.
...
[NOTE]
If the book already exists, CommandException(MESSAGE_DUPLICATE_BOOK) will be thrown. It will not call `Model#commitBookShelf()`, so the address book state will not be saved into the `BookShelfStateList`.
...
Step 3. The `AddReviewCommandParser` parses this command, and returns an `AddReviewCommand`. Exception will be thrown if the command is invalid.

Step 4. The `AddReviewCommand` creates a `Review` object based on the parameter the user keyed in.

Step 5. The `AddReviewCommand` executes. The command checks if the book to which the review is added exists in the `BookShelf`, based on `BookNameContainsExactKeywordPredicate`. If the book does not exist, an exception is thrown. If the book exists, the `Review` object is added to the set of reviews of the book.

==== Design Considerations

===== Aspect: Data structure to store Reviews

* **Alternative 1 (current choice):** Use a Set to store all Reviews, just like tags.
...
* **Alternative 2:** Use a List to store Reviews
...
* **Alternative 1 (current choice):** Use a Set within a Book, just like tags.
** Pros: Easy to code and manage because it is the same implementation as tags. The design makes sense because Reviews belong to Books.
** Cons: It is not known whether this design is compatible with the UI component.
* **Alternative 2:** Have another ObservableList for Reviews.
** Pros: Maybe makes it easier for the UI to implement.
...
. Add a shorthand equivalent alias for each of the individual commands. For example, besides typing `clear`, the user can also type `c` to remove entries stored, including all books and their reviews.
+
****
* Hints
...
|`* *` |user |entrypt |entrypt your review

|`* *` |user |detrypt |detrypt your entrypted review
...
1.  User requests to delete a book and all its reviews by the book's exact name
...
. Deleting a book by its name

.. Prerequisites: The book is inside the list. User knows the full name of the book.
.. Test case: `delete n/Alice` +
   Expected: The book named Alice is deleted from the list. Details of the deleted contact shown in the status message. Timestamp in the status bar is updated.
.. Test case: `delete 0` +
