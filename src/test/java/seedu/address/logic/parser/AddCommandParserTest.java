...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        Person expectedPerson = new PersonBuilder(BOB).changeSkills(VALID_SKILL_PYTHON).withDegree(VALID_DEGREE_BOB)
                .build();

        // whitespace only preamble
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB
                + EDUCATION_DESC_BOB + GPA_DESC_BOB + DEGREE_DESC_BOB
        ...
        Person expectedPersonMultipleTags = new PersonBuilder(BOB).withSkills(VALID_SKILL_PYTHON, VALID_SKILL_JAVA)
                .build();
        assertParseSuccess(parser, NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + EDUCATION_DESC_BOB
                + GPA_DESC_BOB + DEGREE_DESC_BOB + ADDRESS_DESC_BOB
                + SKILL_DESC_JAVA + SKILL_DESC_PYTHON, new AddCommand(expectedPersonMultipleTags));
    }

    @Test
    public void parse_optionalFieldsMissing_success() {
        // zero tags
        Person expectedPerson = new PersonBuilder(AMY).changeSkills().build();
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY + EMAIL_DESC_AMY + EDUCATION_DESC_AMY
                        + GPA_DESC_AMY + DEGREE_DESC_AMY + ADDRESS_DESC_AMY, new AddCommand(expectedPerson));
    }
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, INVALID_NAME_DESC + PHONE_DESC_BOB + EMAIL_DESC_BOB + INVALID_ADDRESS_DESC
                + EDUCATION_DESC_BOB + GPA_DESC_BOB + DEGREE_DESC_BOB, Name.MESSAGE_CONSTRAINTS);

        // non-empty preamble
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB
                + EDUCATION_DESC_BOB + GPA_DESC_BOB + ADDRESS_DESC_BOB + DEGREE_DESC_BOB + SKILL_DESC_JAVA
                + SKILL_DESC_PYTHON, String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddCommand.MESSAGE_USAGE));
    }
}
