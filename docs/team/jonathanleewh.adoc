...
* *Minor enhancement*: As there are currently no third party libraries available which validates file name such that it adheres to the file naming conventions of the major operating systems (OS), I have created a `FileName` class which implements this feature to complement the `export` feature. It requires an indepth knowledge on the different file naming conventions of the various OS and also numerous testing as I discovered that some file name conventions were not documented and not allowed to be used as file names.
...
*** Added Frequently Asked Question (FAQ) section to User Guide: https://github.com/CS2103-AY1819S2-T12-3/main/pull/123[#123]
...
include::../UserGuide.adoc[tag=filenamingconvention]
...
include::../DeveloperGuide.adoc[tag=validationoffilename]
