...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `RequestListPanel`, `InfoPanel`, `HealthWorkerListPanel`, `StatusBarFooter`, etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
The `add`, `edit` and `delete` command have been tweaked from the original AB4
 to include addition of and `Request` and `HealthWorker`.
