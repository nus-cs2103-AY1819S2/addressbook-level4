...
The `Command` component parses the user input into a postal code. It then checks if the postal code is inside the `PostalDataSet`
If the postal code is valid, it will creates a new `SortDistanceComparator<Restaurant>` class with the postal code inputted as the parameters.
This `SortDistanceComparator` is then passed to the `Model` class to sort the `SortedList` which encapsulates the `FilteredList`.
Otherwise, if the postal code provided is invalid, it will simply filter out unreviewed restaurants.
