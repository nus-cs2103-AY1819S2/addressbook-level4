...
Please continue on to the <<Setting Up, [blue]#Setting Up#>> section to get started. You can also read <<AboutUs#, [blue]#AboutUs.adoc#>>
...
. <<Testing,[blue]#Run the tests#>> to ensure they all pass.
...
. Configure the <<Docs-SiteWideDocSettings, [blue]#site-wide documentation settings#>> in link:{repoURL}/build.gradle[`[blue]#build.gradle#`], such as the `[fuchsia]#site-name#`, to suit your own project.
...
1. Get some sense of the overall design by reading [blue]#<<Design-Architecture>>#.
2. Take a look at [blue]#<<GetStartedProgramming>>#.
