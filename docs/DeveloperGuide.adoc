...
The functionality above is facilitated by the `StudyView` class.
...
* `studyState` - variable which stores whether the user is seeing a question or an answer.

These variables are continuously being altered every time the user interacts with the program, explained in detail later.
...
This section discusses how the two variables, `currentCard` and `studyState is modified.
...
The study mechanism is facilitated by `StudyView`.
