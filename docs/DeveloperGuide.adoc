...
[NOTE]
====
The "ship management" feature was named as such to avoid confusion with the title of the application, Battleship CLI.
====
...
The `put` command puts a ship on the map grid, specified by the __head coordinate__.

====
*__What is the head coordinate?__*
...
The `list` command can list certain ships with certain tags by taking the filtering the list of deployed ships. The code snippet below illustrates this approach.

[source,java]
----
include::../src/main/java/seedu/address/logic/commands/ListCommand.java[lines=61..68]
----
...
The following sequence diagram summarizes what happens at the end of a game and an automatic save and comparison is
performed.

<To Include Sequence Diagram>
