...
Step 3. The `WarningPanel` self-invokes `setUpVBox(ObservableList<Medicine>, ObservableList<Medicine>)` to populate the VBox with `WarningListView` objects for display. There will be two `WarningListView` objects, each initialised with its corresponding `ObservableList<Medicine>`, a list descriptor and the predicate accessor.
