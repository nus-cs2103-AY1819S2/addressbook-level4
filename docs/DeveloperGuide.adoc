...
This can be seen in the example found below:

.Example of NUS Course Requirement
image::nus_requirement.PNG[width ="800"]

For this requirement, the student can choose to complete either CS3203 or
CS3216 and CS3217 or CS3281 and CS3282. We observe that can be broken down into 3 distinct,
...
For instance in `Fig 9.`, a suitable `Condition` for completing CS3216 and CS3217 would be
...
When `displayreq` command is called by the user, the sequence of execution is outlined below:
...
** Cons: Can be difficult to find the correct regular expression for some `Condition`

* **Alternative: `Condition` class stores a list of `String` to check whether a condition is satisfied**
