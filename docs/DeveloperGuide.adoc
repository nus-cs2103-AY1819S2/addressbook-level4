...
IMPORTANT: Out of the three key operations stated above, *only* `CsvWrapper#export()` is a _public_ operation available for use by other components. It acts as the main interface which other components use to interact with `CsvWrapper` in order to integrate exporting to CSV file format feature into their own implementation.
...
[NOTE]
If a CSV file with the input file name *already exist* in the `exported` directory, a `File already exist` error will be shown in the `CommandResult` panel of the GUI as a `CommandException` is thrown and the exporting process will stop executing. In other words, the current Medicine Inventory data shown in the GUI would not be exported.
...
(For all use cases below, the *System* is the `MediTab` and the *Actor* is the `pharmacist`, unless specified otherwise)
...
[none]
* 1a. Pharmacist used a `filter` function, filtered inventory is shown.
+
[none]
** 1a1. Pharmacist requests for complete inventory to be listed.
...
[discrete]
=== Use Case 2: Update Inventory in Batches Medicine

*MSS*

1.  Pharmacist notes the index of the selected medicine in the list.
2.  Pharmacist request to add new batch to medicine using the index.
3.  System requests details of the new batch of medicine
4.  Pharmacist enters the required details
5.  System displays updated details
+
Use case ends.
...
+
[none]
** 1a1. Pharmacist uses the `search` function to look for medicine listing.
...
[none]
* 1b. Medicine is new and has not been added to the inventory.
+
[none]
** 1b1. Pharmacist uses the `add` function to add a new entry to list.
** 1b2. Pharmacist enters details of the medicine to add.
** 1b3. Pharmacist notes the index of the medicine listing from the search result.
+
Use case resume at step 2.
...
1.  Pharmacist uses the `delete` function to remove a listing from the inventory.
...
[none]
* 1a. Selected entry is not empty. Deletion is illegal.
+
[none]
** 1a1.  System displays error message.
** 1a2.  Pharmacist uses the `edit` function to decrease quantity of medicine to zero.
** 1a3.  System displays updated quantity.
** 1a4.  Repeat step 1.
+
Use case resume at step 2.
