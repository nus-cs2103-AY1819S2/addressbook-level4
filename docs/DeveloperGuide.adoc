...
A `StudyDeckCommand` is issued after reading the command words, `study INDEX`.

Upon the execution of this command, Model's `viewState` will now hold a `StudyView` object.

As part of the execution of `StudyDeckCommand`, Step 2 is ran.

Step 2. Generating a question.

In order to present questions to users, these are the functions called by the command:

* `DeckSuffler#generateCard()` The `DeckShuffler` class a well-crafted algorithms (implementation explained later)
to select a card a Card object, and passes this reference to `StudyView`.
 This modifies `currentCard` to a random Card object based on the Card generated.

* `currentState` is set to QUESTION.

* Besides these two, `textShown` property is also modified. In this case, it will contain
the question String of the chosen card.

Step 2. User executes the `[wildcard]` command to signal that he has come up with the answer.

This changes the state to answer state.
Consequently, we also change the `textShown` property to reflect this change.

Step 3. This command calls `Card#getAnswer()`
This changes the `textShown` property in the UI.
The `getState` property in the UI is also modified to Answer enum.

Step 4. User enters the difficulty rating command.
User answer is stored as command under the stastistics variable. Since we are mutating the app,
...
modify the colours of flash cards in the UI. This variables also specifies whether or not there
is as we currently they can't good access important websites.
