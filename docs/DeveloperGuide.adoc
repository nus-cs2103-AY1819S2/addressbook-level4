...
image:BattleshipHeadExample.png[width=400]
====
...
The boundary checks are essential to the functionality of both the human and computer player. This is because `BoundaryValueChecker` is used by the human player in the `put` command, while the computer player when it is deploying its own ships after the human player has ended his or her turn. `BoundaryValueChecker` relies on four separate methods to perform its checks. They are:
...
Once `BoundaryValueChecker` has finished its checks and if there are no exceptions thrown, the `put` command proceeds to call the `putShip()` method of the `mapGrid` object to put the ship on the map grid. It then updates the `Fleet` in the human player. The following sequence diagram illustrates what happens when the `put` command is called.
...
[source,java]
----
include::../src/main/java/seedu/address/model/Player/Fleet.java[lines=170..180]
----

The sequence diagram below shows how the tags are listed.
...
image::ListShipSequenceDiagram.png[]
...
image::ShipManagementActivityDiagram.png[]
