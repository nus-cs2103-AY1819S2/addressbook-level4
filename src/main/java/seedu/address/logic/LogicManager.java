...
public class LogicManager implements Logic {
    ...
    private boolean foodDiaryModified;
    ...
    public LogicManager(Model model, Storage storage) {
        ...
        // Set foodDiaryModified to true whenever the models' food diary is modified.
        model.getFoodDiary().addListener(observable -> foodDiaryModified = true);
    }

    @Override
    public CommandResult execute(String commandText) throws CommandException, ParseException {
        logger.info("----------------[USER COMMAND][" + commandText + "]");
        foodDiaryModified = false;
        ...
        if (foodDiaryModified) {
            logger.info("Food diary modified, saving to file.");
            try {
    ...
    public ReadOnlyProperty<Restaurant> selectedRestaurantProperty() {
        return model.selectedRestaurantProperty();
    }

    @Override
    public void setSelectedRestaurant(Restaurant restaurant) {
        model.setSelectedRestaurant(restaurant);
    }
}
