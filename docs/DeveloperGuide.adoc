...
Whenever the `rec` command is called, a `RecModuleManager` is created which generates a `RecModulePredicate` and
a `RecModuleComparator` given the user's `Course` and `ModulePlan`. This ensures that the recommendation list is updated
according to any changes in `Course` and `ModulePlan`.

The `RecModulePredicate` is applied to a `FilteredList` of `ModuleInfoCode` stored in `ModelManager`, to remove modules which
...
A mapping of `ModuleInfoCode` to this `CourseReqType` is put into a `HashMap` stored in `RecModuleManager`.
Subsequently, within each comparison by the `RecModuleComparator`, the `CourseReqType` satisfied by the two modules are
retrieved from the `HashMap` and compared.
