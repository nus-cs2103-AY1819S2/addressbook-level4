...
* can load `ModuleInfo`, `Course` in json format into `Model`.
...
Some of the course requirements in NUS are made up of smaller course requirements.
This can be seen in the requirement found below:

image::nus_requirement.PNG[width ="800"]

For this requirement, the student can choose to complete either CS3203 or
CS3216 and CS3217 or CS3281 and CS3282. We observe that this requirement is composed of 3 distinct,
simpler requirements.

Hence to model NUS course requirement accurately, the `CourseRequirement` interface follows a `Composite` design pattern. This is favoured as it allows
us to treat individual and composition of `CourseRequirement` objects uniformly.
//TODO class diagram
...
list of `Condition` objects and each `Condition` object stores a `Pattern` and an `int`, `minToSatisfy`. A `Condition`
...
that tells the object how two different `CourseRequirement` are composed using logical operations. For instance, for
...
of the two.
//TODO elaborate more on AND
* `getUnfulfilled` -- a method that accepts a list of `ModuleInfoCode` and returns a list of RegExes that none of the
`ModuleInfoCode` satisfies. This method is used in the recommend feature.

===== `RequirementStatus` Class
The `RequirementStatus` is an association class that links a list of `ModuleInfoCode` and `CourseRequirement`.
In addition to this two objects, the `RequirementStatus` class also stores the result when the `isFulfilled` and
`percentageFulfilled` of the `CourseRequirement` it stores acts on its list of `ModuleInfoCode`.

The `RequirementStatus` is also the class used to display the results when the `displayreq` command is called.

When `displayreq` command is called by the user, the sequence of execution is as follows:

1. `Model#updateRequirementStatusList` is called. This clears the pre-existing `RequirementStatusList`
and fills it with new `RequirementStatusObject` based on current `Course` and `ModulesTakenList`.
...
`ModuleTaken`**
** Pros: Easier to implement since we are restricting scope to only checking whether strings match
pattern in `Condition`
** Cons: There could be corner cases that we left out; actual course requirement that we check by
only checking the module code.
...
** Cons: More Java classes/coding required also means that more tests
and storage components for different classes.

We chose the current method over the alternative because it is requires much fewer classes to implement.
...
The module recommendation feature displays modules which the user is recommended to take based on existing GradTrak modules
...
image::RecModuleClassDiagram.png[width="500"]
...
.. It first tests if the `ModuleInfoCode` (call it `codeToTest`) in a `RecModule` is eligible to be taken,
using an `EligibleModulePredicate` which takes in `GradTrak`. Those `RecModule` corresponding to `ModuleTaken` in `GradTrak`
which are already passed or to be taken in a future semester (collectively known as non-failed modules) are filtered out at this stage.
...
considering that lower level modules are usually taken first. In the case of equal priority and module level, lexicographical sorting is used.
