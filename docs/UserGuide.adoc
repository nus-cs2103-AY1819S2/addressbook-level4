...
_Figure 1: After typing `list`_. Oh no the categories for Aston's are wrong! It is definitely not Chinese! Let's change it!
...
* You can key in categories in any order. e.g. Entering `Price Range` before `Cuisine` will
work as well.
* You can set any number of categories at once. You have complete freedom to set categories
...
* You do not need to be concerned about which categories you want to filter. Filtering works across categories and also
within categories of the same type.
