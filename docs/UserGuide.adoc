...
TeethHub automatically saves all data in data.json when you close the program.
When 'data' is mentioned, it refers to the the patient list and their respective records plus the task list.
However, you can manage them manually with Open, Save, Import and Export. +
 +
You can use *open* to open another data that you may have transferred over from another computer. +
 +
You can use *save* to save your current data to another .json, so that you can transfer them to another computer. +
You can also use *save* to save your current data to a .pdf, which is nicer on the eyes.
 +
You can use *Import* to add select patients from another data to your current data. +
 +
...
* `open data1.json` +
Replaces the current data with the data in "patients1.json".

* `open february\data1.json` +
Replaces the current data with the data in "patients2.json" found in the "february" folder.
...
* `save patients1.json` +
Saves the current data to "patients1.json".

* `save february\patients2.pdf` +
Saves the current data to "patients2.json" found in the "february" folder.


==== import
Opens the file in the specified file path and adds the specified patients to the data.
...
Adds patient 1 from "patients1.json" to the current data.

* `import february\patients1.json 1` +
Adds patient 1 from "patients1.json" found in the "february" folder to the current data.

* `import patients1.json 1,4` +
Adds patient 1 and patient 4 from "patients1.json" to the current data.

* `import patients1.json 1-4` +
Adds patient 1 to patient 4 from "patients1.json" to the current data.

* `import patients1.json 1,3-5` +
Adds patient 1 and patient 3 to patient 5 from "patients1.json" to the current data.
...
Saves patient 1 from the current data to "patients1.json".

* `export february\patients1.pdf 1` +
Saves patient 1 from the current data to "patients1.pdf" found in the "february" folder.

* `export patients1.json 1,4` +
Saves patient 1 and patient 4 from the current data to "patients1.json".

* `export patients1.pdf 1-4` +
Saves patient 1 to patient 4 from the current data to "patients1.pdf".

* `export patients1.pdf 1,3-5` +
Saves patient 1 and patient 3 to patient 5 from the current data to "patients1.pdf".

* `export patients1.json all` +
Writes all entries from the current data to patients1.json. +
...
*A*: Install TeethHub in computer B and copy over the TeethHub.json
in the data folder of computer A to the data folder of computer B.
Alternatively, upload your data to the cloud and
...
* *Export* : `export FILEPATH INDEX_RANGE`` +
e.g. `export \folder\data.json 1,3-5`
...
* *Import* : `import FILEPATH INDEX_RANGE` +
e.g. `import \folder\data.json 1,3-5`
