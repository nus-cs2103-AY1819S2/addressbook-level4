...
:: *Step 1:* By default, the charts are displayed when TravelBuddy launches. To navigate away from the charts, type in `select 1`, as seen in <<generateCommand1Figure>>. +
*Outcome:* The first place in the place list will be selected and displayed on the right-hand side of the panel, as shown in <<generateCommand2Figure>>.
...
:: *Step 2:* Type in `generate` to generate the charts, as seen in <<generateCommand2Figure>>. +
*Outcome:* The charts will be displayed on the right-hand side of the panel, as shown in <<generateCommand3Figure>>.
...
*Example:* :: <<generateCommand7Figure>> shows the state of the bar charts before the command `edit 1 cc/USA` was executed, while <<generateCommand8Figure>> shows the state of the bar chart, with USA included, after the `edit` command was executed.
