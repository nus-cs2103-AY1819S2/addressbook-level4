...
* Ensure that files created with file names specified by the user can be used on Windows, Linux and MacOS operating systems. This is important in order to avoid potential bugs involving file names as MediTabs supports Windows, Linux and MacOS operating systems hence, the user might switch between these operating systems when using MediTabs.

Examples of commands in MediTabs which uses the `FileName` class to validate file name specified by the user:
