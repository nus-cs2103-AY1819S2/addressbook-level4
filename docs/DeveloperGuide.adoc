...
inconvenience caused if you accidentally decide on a timing outside of office hours.
...
month, they do not have to check which is the last date of the month in order to specify the end date.
...
list of appointments is modified, which might not be straightforward to implement.
. The benefit of a permanent list of free slots is the quicker execution time of `freeapp`, which will typically
only be called a small number of times (around 10) a day, when booking an appointment slot with a patient. The total
time saved on executing `freeapp` is therefore negligible.
