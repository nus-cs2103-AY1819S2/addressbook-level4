...
* `CurrentEdit#getTempImage` - Retrieves the temporary image `tempImage` which stores the filepath of the temporary image in the `temp` folder, its history of edits [`List<Command>`] and its metadata.

* `CurrentEdit#updateTempImage` - Replaces the temporary image in `temp` folder with the newly edited image and update the `tempImage` instance in the class.
