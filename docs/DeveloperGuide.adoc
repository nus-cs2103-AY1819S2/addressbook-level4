...
Step 3. The `WarningCommand#execute` method is invoked. The type and value of the new threshold is passed into `Model#changeWarningPanelListThreshold`.
...
The activity diagram below summarises what happens when a user execute the `warning` command:
