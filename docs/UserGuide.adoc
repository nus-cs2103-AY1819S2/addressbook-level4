...
[NOTE]
=====
Undoable commands: those commands that modify slaveFinder()'s content (`add`, `delete`, `edit`, `clear`, `createJob`, `deleteJob`, `generateInterviews`, `setMaxInterviewsADay`, `setBlockOutDates`, `clearInterviews`, `filter`, `delete filter`).
=====
