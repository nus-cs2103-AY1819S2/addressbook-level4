...
[discrete]
===== Model
The model component supports the addition of a new object attribute [red]#CsvManager#, that carries out the read and
write logic of cards to external files.

Additionally, model implements two new methods, [red]#Model#importCardFolders# and [red]#Model#exportCardFolders#.
These two methods further call the CsvManager API to read and write card folders.

Model also implements [red]#Model#setTestCsvPath# and the [red]#Model#getDefaultPath# which are only used during testing.
...
* [red]#CsvManager# - Main class responsible for the writing and reading of .csv files during the import and export of files
* [red]#CsvFile# - Wrapper class for file name input by user
* [red]#CsvCommands# interface - API for import/export method in CsvManager
* [red]#CsvUtils# - Utility class containing the default path and test paths used by the application.

The main logic for the import / export command is carried out inside the CsvManager class specified by it's
API [red]#CsvManager#writeFoldersToCsv# and [red]#CsvManager#readFoldersToCsv#.
...
2. The Export command parser extracts the indices, stores the indices into a [red]#List<Integer># and
[red]#ExportCommand# object, which keeps track of the indices.


3. The Export command calls executes, resulting in a call to the  [red]#Model#exportCardFolders# method.

4. [red]#Model#exportCardFolders# method checks that all card folders specified by user exists within the model.
   ...
   The method throws [red]#CardFolderNotFoundException# if card folder index is not found in list.
+
image::import_export_model_list1.png[width = "800"]


5.  [red]#CsvManager# is called to carry out the main logic of writing the card folders to file.
...
7. Import command parser extracts file name, wraps file into a [red]#CsvFile# object and
   parses the [red]#CsvFile# object into an Import Command object.

8. Logic unit executes the import command.
The execute method makes a call to [red]#Model#importCardFolder# method. The model checks that
the card folder to import does not already exist within the model. Throws [red]#DuplicateCardFolderException#
if card folder already exists.

9. CsvManager is called to carry out the main logic of reading card folders to file.
File imported will be serialized into a [red]#CardFolder# object and
added to the [red]#folders# variable in model.
