...
==== Current Implementation
* The `MemberSetActive` command sets the member status to active. The member's personCard will then be updated based on
the colour allocated to the active status. Active status denotes that is still an active member of the CCA.

* If a member is already active, `MemberSetActive` will inform user that the user is already active. No action taken.

=== MemberSetInactive

==== Current Implementation
* The `MemberSetInactive` command sets the member status to inactive. The member's personCard will then be updated based on
the colour allocated to the inactive status. Inactive status denotes that the member has graduated from school.

* If a member is already inactive, `MemberSetActive` will inform user that the user is already active. No action taken.
...
* For future implementation, the `memberSort` command will sort the members based on active and inactive status (second priority).
Active and inactive members will have their personCard differentiated by color.
// end::membersort[]
...
[discrete]
=== Use case: Sort Activity

*Main Success Scenario (MSS)*

1.  User requests to sort the activity list
2.  Club Manager sorts the activity list according to date
3.  Club Manager shows the sorted list
+
Use case ends.
...
1. User requests to list activities
2. Club Manager shows a list of activities
3. User request to archive an activity from the displayed list
4. Club Manager archives the selected activity
+
Use case ends.
...
* 3b. The activity at the given index is already archived.
+
[none]
** 3b1. Club Manager shows an error message.
+
...
[discrete]
=== Use case: Unarchive Activity

* Main Success Scenario (MSS)*

1. User requests to list activities
2. Club Manager shows a list of activities
3. User request to unarchive an activity from the displayed list
4. Club Manager unarchives the selected activity
+
Use case ends.
...
* 3b. The activity at the given index is not archived.
+
[none]
** 3b1. Club Manager shows an error message.
+
