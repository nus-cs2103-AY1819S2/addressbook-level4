...
The warning panel is driven by the `MainWindow` which creates `WarningPanel` that extends `UiPart`. The `WarningPanel` contains two `WarningListView` objects, where the lists for expiring medicine and medicine low in stock will reside. Both lists updates in real time when any changes made to the inventory. Each item of a list is represented by a `WarningCard` object.
